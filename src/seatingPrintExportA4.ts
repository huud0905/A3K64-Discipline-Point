...
function spa4PrintHtml(seats: Spa4Seats, changeNo: string, day: string, month: string, year: string) {
  const no = spa4Escape(changeNo || "...");
  const d = spa4Escape(day || "...");
  const m = spa4Escape(month || "...");
  const y = spa4Escape(year || "...");
  return `<!doctype html>
<html lang="vi">
<head>
<meta charset="utf-8" />
...
<style>
  ...
  *{box-sizing:border-box;}
  html,body{margin:0;padding:0;background:#fff;color:#000;font-family:"Times New Roman",Arial,"Noto Sans",serif;}
  body{width:277mm;min-height:190mm;}
  .page{width:100%;min-height:190mm;position:relative;padding:7mm 8mm 8mm;}
  .title{text-align:center;color:red;font-size:19pt;font-weight:700;margin:0 0 4mm;text-transform:uppercase;}
  .subtitle{text-align:center;color:red;font-size:16pt;font-weight:700;margin:0 0 2mm;}
  .room{position:relative;border:1.8px solid #0f3554;min-height:145mm;padding:13mm 12mm 9mm;}
  .back{position:absolute;top:4mm;left:50%;transform:translateX(-50%);font-size:14pt;font-weight:700;}
  .layout{display:grid;grid-template-columns:1fr 28mm 1fr;gap:22mm;align-items:stretch;margin-top:1mm;}
  .side{display:grid;grid-template-rows:repeat(7,1fr);gap:9.2mm;align-content:stretch;}
  .desk{width:100%;height:8.4mm;border-collapse:collapse;table-layout:fixed;font-size:13.5pt;}
  .desk td{border:1px solid #000;text-align:center;vertical-align:middle;height:8.4mm;padding:0 2mm;white-space:nowrap;line-height:1.1;}
  .aisle{display:flex;align-items:flex-end;justify-content:center;font-size:13pt;font-weight:700;padding-bottom:1mm;}
  .front{display:grid;grid-template-columns:1fr 28mm 1fr;gap:22mm;margin-top:2mm;font-size:14pt;font-weight:700;}
  .front span{text-align:center;}
  .gv{position:absolute;right:9mm;bottom:2mm;font-size:13.5pt;}
...
<body>
  <main class="page">
    ...
    <h2 class="subtitle">Thay đổi lần ${no} – Áp dụng từ ngày ${d} tháng ${m} năm ${y}</h2>
    <section class="room">
...
function spa4OpenPrint(changeNo: string, day: string, month: string, year: string) {
  const seats = spa4CurrentSeats();
  const html = spa4PrintHtml(seats, changeNo, day, month, year);
...
function spa4ShowModal() {
  ...
  wrap.innerHTML = `
    <div class="spa4-card" role="dialog" aria-modal="true">
      ...
      <div class="spa4-grid">
        ...
        <label>Ngày<input data-field="day" value="${spa4Escape(t.day)}" inputmode="numeric" /></label>
        <label>Tháng<input data-field="month" value="${spa4Escape(t.month)}" inputmode="numeric" /></label>
        <label>Năm<input data-field="year" value="${spa4Escape(t.year)}" inputmode="numeric" /></label>
      </div>
  ...
  wrap.querySelector<HTMLButtonElement>("[data-print]")?.addEventListener("click", () => {
    ...
    spa4OpenPrint(get("change"), get("day"), get("month"), get("year"));
    wrap.remove();
  });
...
function spa4Style() {
  ...
  style.textContent = `
    ...
    #${SPA4_MODAL_ID} .spa4-card{width:min(560px,100%);background:rgba(255,255,255,.98);border:1px solid #cbd5e1;border-radius:24px;box-shadow:0 30px 100px rgba(15,23,42,.24);padding:22px;color:#0f172a;}
    ...
    #${SPA4_MODAL_ID} .spa4-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;}
    #${SPA4_MODAL_ID} label{display:grid;gap:6px;font-size:12px;font-weight:950;color:#334155;}
    #${SPA4_MODAL_ID} input{height:42px;border:1px solid #cbd5e1;border-radius:14px;padding:0 12px;font-size:15px;font-weight:900;outline:none;}
    ...
    .theme-dark #${SPA4_MODAL_ID}{background:rgba(0,0,0,.45);}.theme-dark #${SPA4_MODAL_ID} .spa4-card{background:#0f172a;color:#f8fafc;border-color:#334155;}.theme-dark #${SPA4_MODAL_ID} p,.theme-dark #${SPA4_MODAL_ID} label{color:#cbd5e1;}.theme-dark #${SPA4_MODAL_ID} input,.theme-dark #${SPA4_MODAL_ID} button{background:#111827;color:#f8fafc;border-color:#334155;}
  `;
