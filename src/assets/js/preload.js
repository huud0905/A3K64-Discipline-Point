...
(function (global) {
  ...
  // profile.js (cửa sổ profile-window.html) — GAS_CACHE_KEY, TTL 20s
  // (xem readGasCache()/writeGasCache() trong profile.js). Format phải
  // khớp: { data, ts }. Ghi cả sessionStorage lẫn localStorage — bắt
  // buộc có localStorage vì cửa sổ Profile là BrowserWindow riêng,
  // không đọc được sessionStorage của cửa sổ login.
  const KEY_PROFILE_GAS     = 'a3k64-profile-gas-cache-v1';
  ...
  /** Ghi scoreboard vào cache mà profile.js đọc (readGasCache()).
   *  Format phải khớp: { data, ts } — data là remote {students,events,weeks},
   *  ts dùng để so TTL 20s trong profile.js.
   *  Ghi CẢ HAI storage: sessionStorage (cửa sổ login đọc lại nhanh nếu
   *  vẫn ở đó) VÀ localStorage — bắt buộc phải có localStorage vì
   *  sessionStorage không chia sẻ được giữa cửa sổ login và cửa sổ
   *  Profile (2 BrowserWindow khác nhau trong Electron); thiếu dòng
   *  localStorage thì preload chạy xong Profile vẫn không thấy gì. */
  function writeProfileCache(remote) {
    if (!remote?.students?.length) return;
    const payload = { data: remote, ts: Date.now() };
    safeSet(sessionStorage, KEY_PROFILE_GAS, payload);
    safeSet(localStorage,   KEY_PROFILE_GAS, payload);
  }
    ...
    const taskScoreboard = gasGet(gasUrl, { action: 'getScoreboard' })
      .then(function (json) {
        ...
        if (remote?.students?.length) {
          ...
          writeProfileCache(remote);
          console.log('[Preload] ✓ Scoreboard (' + (Date.now() - startTime) + 'ms)');
          console.log('[Preload] ✓ Profile (' + (Date.now() - startTime) + 'ms)');
        }
      })
