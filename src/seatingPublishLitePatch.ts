...
type SeatPubLiteStatus = "private" | "published";
...
function seatPubLiteCleanStatus(value: unknown): SeatPubLiteStatus {
  return String(value || "").trim() === "published" ? "published" : "private";
}
...
function seatPubLiteSaveLocal(data: SeatPubLiteConfig) {
  localStorage.setItem(SEAT_PUB_LITE_KEY, JSON.stringify({ ...data, publishAt: "", previewStudents: "" }));
}
...
function injectSeatPubLiteStyle() {
  ...
  style.textContent = `
    ...
    .seat-pub-lite-modal h3{margin:0;font-size:20px;font-weight:1000}.seat-pub-lite-modal p{margin:0;color:#64748b;font-size:13px;line-height:1.45}.seat-pub-lite-meta{font-size:12px;color:#64748b;font-weight:800}.seat-pub-lite-status-pill{display:inline-flex;align-items:center;justify-content:center;min-width:92px;height:30px;border-radius:999px;font-weight:1000;font-size:12px;border:1px solid #cbd5e1;background:#f8fafc;color:#0f172a}.seat-pub-lite-status-pill.on{background:#dcfce7;color:#166534;border-color:#86efac}
    .seat-pub-lite-grid{display:grid;gap:10px}.seat-pub-lite-label{display:grid;gap:6px;font-weight:950;font-size:13px}
    ...
    .theme-dark .seat-pub-lite-modal{background:#0f172a;color:#f8fafc;border-color:#334155;box-shadow:0 28px 90px rgba(0,0,0,.42)}.theme-dark .seat-pub-lite-modal p,.theme-dark .seat-pub-lite-meta,.theme-dark .seat-pub-lite-manage-sub{color:#94a3b8}.theme-dark .seat-pub-lite-trigger,.theme-dark .seat-pub-lite-actions button,.theme-dark .seat-pub-lite-mini-btn{background:#111827;color:#f8fafc;border-color:#334155}.theme-dark .seat-pub-lite-menu{background:#1f2937;border-color:#334155}.theme-dark .seat-pub-lite-option{color:#f8fafc}.theme-dark .seat-pub-lite-option:hover,.theme-dark .seat-pub-lite-option.active{background:#334155}.theme-dark .seat-pub-lite-manage-row{background:#111827;border-color:#334155}.theme-dark .seat-pub-lite-status-pill{background:#111827;color:#f8fafc;border-color:#334155}.theme-dark .seat-pub-lite-status-pill.on{background:#064e3b;color:#bbf7d0;border-color:#047857}
  `;
...
function renderStatusSelect(status: SeatPubLiteStatus) {
  return `<div class="seat-pub-lite-select"><button type="button" class="seat-pub-lite-trigger"><span>${statusText(status)}</span><span>⌄</span></button><div class="seat-pub-lite-menu"><button type="button" class="seat-pub-lite-option" data-value="private">Riêng tư</button><button type="button" class="seat-pub-lite-option" data-value="published">Công bố ngay</button></div></div>`;
}

function bindStatusSelect(backdrop: HTMLElement, getStatus: () => SeatPubLiteStatus, setStatus: (value: SeatPubLiteStatus) => void) {
...
function openSeatPubLiteModal() {
  ...
  backdrop.innerHTML = `
    <div class="seat-pub-lite-modal">
      ...
      <div class="seat-pub-lite-grid"><label class="seat-pub-lite-label">Trạng thái${renderStatusSelect(status)}</label></div>
  ...
  bindStatusSelect(backdrop, () => status, (value) => { status = value; });
...
async function openSeatPubManageModal() {
  ...
  backdrop.innerHTML = `
    <div class="seat-pub-lite-modal">
      ...
      <div class="seat-pub-lite-manage-row">
        <div><div class="seat-pub-lite-manage-title">Sơ đồ hiện tại</div><div class="seat-pub-lite-manage-sub">${formatMeta(config)}</div></div>
        <span class="seat-pub-lite-status-pill ${config.status === "published" ? "on" : ""}">${statusText(config.status)}</span>
      </div>
      <div class="seat-pub-lite-manage-actions"><button class="seat-pub-lite-mini-btn" data-private>Riêng tư</button><button class="seat-pub-lite-mini-btn primary" data-publish>Công bố</button><button class="seat-pub-lite-mini-btn" data-edit>Sửa</button><button class="seat-pub-lite-mini-btn" data-reset>Đặt lại</button></div>
