...
type AppKey = "dashboard" | "quickScore" | "ranking" | "contests" | "students" | "settings";
...
function normalizeAccentValue(value: string | null) {
  ...
  return ACCENT_COLORS[normalized] || null;
}
...
const DESKTOP_SHORTCUTS: AppKey[] = ["dashboard", "students", "settings"];
    ...
    <section className="win-content">
        ...
        <div className="content-hero-top">
          <div>
            <span className="hero-chip">
              <ActiveIcon size={16} /> {active.title}
            </span>
            <h1>12A3 - Quản lý thi đua</h1>
            <p>
              {active.subtitle}. Đường dẫn hiện tại: <b>{active.path}</b>
            </p>
          </div>
          <button type="button" className="hero-action">
            Tạo mới
          </button>
        </div>
      </div>
      ...
      <div className="panel-grid">
        <section className="win-panel">
          <div className="panel-header">
            <div>
              ...
              <span>Cập nhật điểm mới nhất</span>
            </div>
          ...
          <div className="table-like">
            <div className="table-row">
              <div>
                <strong>Phát biểu xây dựng bài</strong>
                <span>Học tập · vừa xong</span>
              </div>
              <div className="score-pill">+5</div>
            </div>
            <div className="table-row">
              <div>
                <strong>Không đeo thẻ học sinh</strong>
                <span>Nề nếp · 15 phút trước</span>
              </div>
              <div className="score-pill negative">-5</div>
            </div>
            <div className="table-row">
              <div>
                <strong>Kiểm tra bài cũ đạt 10</strong>
                <span>Học tập · hôm nay</span>
              </div>
              <div className="score-pill">+10</div>
            </div>
          </div>
        </section>
        ...
        <section className="win-panel">
          <div className="panel-header">
            <div>
              <h2>Lịch & thông báo</h2>
              <span>Nhắc việc trong tuần</span>
            </div>
            <Bell size={18} />
          </div>
          <div className="table-like">
            <div className="table-row">
              <div>
                <strong>Tổng kết thi đua tuần</strong>
                <span>Thứ 7 · 17:00</span>
              </div>
              <CalendarDays size={18} />
            </div>
            <div className="table-row">
              <div>
                <strong>Kiểm tra danh sách vi phạm</strong>
                <span>Lớp trưởng / Bí thư</span>
              </div>
              <ShieldCheck size={18} />
            </div>
          </div>
...
function WindowsDesktop({ user, onLogout }: { user: DesktopUser; onLogout: () => void }) {
  ...
  const [pinnedApps, setPinnedApps] = useState<AppKey[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("pinned-apps") || "[]");
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    ...
    const customUpdate = (event: Event) => {
      const customEvent = event as CustomEvent<Partial<TaskbarSettings>>;
      setTaskbarSettings({
        ...DEFAULT_TASKBAR_SETTINGS,
        ...readTaskbarSettings(),
        ...customEvent.detail,
  ...
  const visibleClock = now.toLocaleTimeString("vi-VN", {
    hour: "2-digit",
    minute: "2-digit",
  });

  const visibleDate = now.toLocaleDateString("vi-VN", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  });
    ...
    setWindows((current) =>
      current.map((item) =>
        item.key === appKey
          ? {
              ...item,
              minimized: restore ? false : item.minimized,
              z: zCounter.current,
            }
          : item
      )
  ...
  const handleTitlebarMouseDown = (event: React.MouseEvent<HTMLElement>, win: WindowState) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      updateWindowPos(win.key, startPos.x + moveEvent.clientX - startX, startPos.y + moveEvent.clientY - startY);
    };
  ...
  const taskbarItems = Array.from(new Set([...pinnedApps, ...windows.map((item) => item.key)]));
  ...
  return (
    <main className={`win-root theme-${resolvedTheme} ${focusMode ? "focus-mode" : ""}`} style={{ "--desktop-accent": accent } as React.CSSProperties}>
      ...
      <section className="win-desktop login-push-enter">
          ...
          {DESKTOP_APPS.filter((app) => DESKTOP_SHORTCUTS.includes(app.key)).map((app) => {
            ...
            return (
              <button
                ...
                draggable
                onDragStart={(event) => event.dataTransfer.setData("text/plain", app.key)}
                onDoubleClick={() => openApp(app)}
                title={locked ? "Bạn chưa có quyền xem mục này" : `${app.title} - bấm đúp để mở`}
              >
        ...
        {windows.length === 0 && (
          <div className="win-empty-note">
            <UserRound size={40} color="var(--desktop-accent)" />
            <h1>Desktop 12A3</h1>
        ...
        {windows.map((win) => {
          ...
          return (
            <section
              key={win.key}
              className={`win-window ${win.maximized ? "maximized" : ""} ${win.minimized ? "minimized" : ""} ${isFocused ? "focused" : ""}`}
              style={
                {
                  "--win-x": `${win.x}px`,
                  "--win-y": `${win.y}px`,
            ...
            >
              <header
                ...
                onMouseDown={(event) => handleTitlebarMouseDown(event, win)}
              ...
              >
                ...
                <div className="window-actions" onMouseDown={(event) => event.stopPropagation()}>
                  <button type="button" title="Thu nhỏ" onClick={() => minimizeWindow(win.key)}>
                    <Minus size={16} />
                  </button>
                  <button type="button" title={win.maximized ? "Khôi phục" : "Phóng to"} onClick={() => toggleMaximize(win.key)}>
                    {win.maximized ? <Minimize2 size={15} /> : <Maximize2 size={15} />}
                  </button>
                  <button className="close" type="button" title="Đóng" onClick={() => closeWindow(win.key)}>
                    <X size={16} />
                  </button>
                </div>
              </header>

              <div className={`win-body ${app.key === "settings" || app.key === "dashboard" ? "settings-mode" : ""}`}>
                {app.key !== "settings" && app.key !== "dashboard" && (
                  <aside className="win-sidebar">
                    <div className="user-card">
                      <div className="avatar">
                        {user.photoURL ? <img src={user.photoURL} alt="Avatar" /> : getInitials(user.displayName)}
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <strong>{user.displayName || "Người dùng 12A3"}</strong>
                        <span>{user.email || user.role || "Đang đăng nhập"}</span>
                      </div>
                    </div>
                      ...
                      {DESKTOP_APPS.map((navApp) => {
                        ...
                        return (
                          <button
                            key={navApp.key}
                            type="button"
                            className={`side-item ${app.key === navApp.key ? "active" : ""} ${locked ? "locked" : ""}`}
                            style={{ "--app-accent": accent } as React.CSSProperties}
                            onClick={() => openApp(navApp)}
                          >
                            <div className="side-item-icon">{locked ? <Lock size={16} /> : <Icon size={16} />}</div>
                            <div style={{ minWidth: 0 }}>
                              <strong>{navApp.title}</strong>
                              <span>{navApp.subtitle}</span>
                            </div>
                          </button>
                        );
                      })}
      ...
      <nav
        className={`taskbar align-${taskbarSettings.alignment} ${taskbarSettings.autoHide ? "auto-hide" : ""}`}
        aria-label="Windows 11 taskbar"
        onDragOver={(event) => event.preventDefault()}
        onDrop={(event) => {
          const appKey = event.dataTransfer.getData("text/plain") as AppKey;
          if (DESKTOP_APPS.some((app) => app.key === appKey) && !pinnedApps.includes(appKey)) {
            setPinnedApps((current) => [...current, appKey]);
          }
        }}
      >
        ...
        <div className="task-center">
          ...
          {taskbarSettings.searchMode === "icon" ? (
            <button
              type="button"
              className="task-icon"
              onClick={() => {
          ...
          <button
            type="button"
            className={`task-focus-switch ${focusMode ? "on" : ""}`}
            onClick={toggleFocusMode}
            title={focusMode ? "Tắt tập trung toàn màn hình" : "Bật tập trung toàn màn hình"}
            aria-pressed={focusMode}
          >
            <span className="task-focus-knob" />
            <span className="task-focus-text">{focusMode ? "ON" : "OFF"}</span>
          </button>

          {taskbarSettings.taskView && (
            <button type="button" className="task-icon" title="Chế độ xem tác vụ">
              <ListChecks size={17} />
            </button>
          )}

          {taskbarSettings.widgets && (
            <button type="button" className="task-icon" title="Tiện ích">
              <Sparkles size={17} />
            </button>
          )}

          {taskbarSettings.resume && (
            <button type="button" className="task-icon" title="Tiếp tục">
              <Home size={17} />
            </button>
          )}
          ...
          {taskbarItems.map((appKey) => {
            ...
            return (
              <button
                key={appKey}
                type="button"
                className={`task-icon ${isFocused ? "active" : ""} ${isOpen ? "running-app" : ""} ${isPinned ? "pinned-app" : ""} ${
                  taskbarSettings.badges && isOpen ? "show-badge" : ""
                }`}
                title={taskApp.title}
                onClick={() => handleTaskbarAppClick(appKey)}
                onContextMenu={(event) => {
                  event.preventDefault();
                  event.stopPropagation();
                  setTaskAppMenu({ x: event.clientX, y: event.clientY, appKey });
                }}
              >
                <TaskIcon size={18} />
              </button>
        ...
        <div className="task-right">
          <span>{visibleClock}</span>
          <span>{visibleDate}</span>
        </div>
      </nav>

      {startOpen && (
        <section className={`start-menu align-${taskbarSettings.alignment}`}>
          ...
          <div className="start-app-grid">
            {DESKTOP_APPS.map((app) => {
              ...
              const locked = !canOpenApp(app, user);
              return (
                <button key={app.key} type="button" className="start-app" onClick={() => openApp(app)}>
                  <div className="start-app-icon">{locked ? <Lock size={18} /> : <Icon size={18} />}</div>
                  <span>{app.title}</span>
                </button>
              );
            })}
          ...
          <div className="start-footer">
            <div className="user-card" style={{ padding: 0, background: "transparent" }}>
              <div className="avatar" style={{ width: 34, height: 34, borderRadius: 12 }}>
                {getInitials(user.displayName)}
              </div>
              <strong>{user.displayName || "12A3"}</strong>
            </div>
            <button type="button" className="logout-button" onClick={onLogout} style={{ width: 118 }}>
      ...
      {searchOpen && (
        <section className={`search-panel align-${taskbarSettings.alignment}`}>
      ...
      {taskAppMenu && menuApp && (
        <div className="task-app-menu" style={{ left: taskAppMenu.x, top: taskAppMenu.y }} onClick={(event) => event.stopPropagation()}>
          <button type="button" onClick={() => togglePinnedApp(menuApp.key)}>
            <span>📌</span>
            {pinnedApps.includes(menuApp.key) ? "Bỏ ghim khỏi taskbar" : "Ghim vào taskbar"}
          </button>
          {windows.some((item) => item.key === menuApp.key) && (
            <button
              type="button"
              onClick={() => {
                closeWindow(menuApp.key);
                setTaskAppMenu(null);
              }}
            >
              <span>×</span>
...
export default function App() {
  ...
  if (!user) {
    return (
      <div className={leavingLogin ? "login-exit-push" : ""}>
        <style>{`
          .login-exit-push {
            animation: loginExitPush .54s cubic-bezier(.2, .8, .2, 1) both;
            transform-origin: center center;
          }

          @keyframes loginExitPush {
            0% { opacity: 1; transform: translateY(0) scale(1); filter: blur(0); }
            100% { opacity: 0; transform: translateY(-34px) scale(1.035); filter: blur(12px); }
          }
        `}</style>
...
const desktopCss = `
  ...
  .win-root {
    ...
    background:
      radial-gradient(circle at 18% 12%, color-mix(in srgb, var(--desktop-accent) 34%, transparent), transparent 32%),
      radial-gradient(circle at 86% 24%, rgba(168, 85, 247, .18), transparent 28%),
      linear-gradient(135deg, #07111f 0%, #111827 48%, #020617 100%);
  }

  .login-push-enter {
    animation: desktopPushIn .64s cubic-bezier(.2, .8, .2, 1) both;
    transform-origin: center bottom;
  }

  @keyframes desktopPushIn {
    0% { opacity: 0; transform: translateY(34px) scale(.965); filter: blur(12px); }
  ...
  .desktop-shortcut-icon svg {
    ...
    height: 34px;
    stroke-width: 2.2;
    filter: drop-shadow(0 8px 18px color-mix(in srgb, var(--desktop-accent) 34%, transparent));
  }
  ...
  .win-window {
    position: absolute;
    left: calc(50% - min(540px, calc((100vw - 176px) / 2)));
    top: 28px;
    ...
    animation: windowOpen .18s ease both;
    will-change: transform;
  }

  .win-window.focused {
    border-color: color-mix(in srgb, var(--desktop-accent) 48%, #273244);
  }
  ...
  .avatar {
    ...
    color: #fff;
    font-weight: 900;
    background: linear-gradient(135deg, var(--desktop-accent), color-mix(in srgb, var(--desktop-accent) 58%, #7c3aed));
    overflow: hidden;
  }
  ...
  .taskbar.align-left .task-center {
  ...
  .task-focus-switch:hover {
    background: #172033;
  }

  .task-focus-switch.on {
    color: #ffffff;
    border-color: transparent;
    background: color-mix(in srgb, var(--desktop-accent) 82%, #0b1220);
  }
  ...
  .task-focus-switch.on .task-focus-knob {
    background: rgba(255,255,255,.18);
  }

  .task-focus-switch.on .task-focus-knob::after {
    background: #ffffff;
  ...
  .win-root::-webkit-scrollbar,
  .win-window::-webkit-scrollbar,
  ...
  .win-root::-webkit-scrollbar-track,
  .win-window::-webkit-scrollbar-track,
  .win-content::-webkit-scrollbar-track,
  .win-body::-webkit-scrollbar-track {
    background: #050914;
  }

  .win-root::-webkit-scrollbar-thumb,
  ...
  .win-root.theme-light .win-window,
  .win-root.theme-light .win-content,
  .win-root.theme-light .win-body {
    scrollbar-color: color-mix(in srgb, var(--desktop-accent, #2563eb) 65%, #94a3b8) #e2e8f0;
  }

  .win-root.theme-light::-webkit-scrollbar-track,
  .win-root.theme-light .win-window::-webkit-scrollbar-track,
  .win-root.theme-light .win-content::-webkit-scrollbar-track,
  .win-root.theme-light .win-body::-webkit-scrollbar-track {
    background: #e2e8f0;
  }

  .win-root.theme-light::-webkit-scrollbar-thumb,
  .win-root.theme-light .win-window::-webkit-scrollbar-thumb,
  .win-root.theme-light .win-content::-webkit-scrollbar-thumb,
  .win-root.theme-light .win-body::-webkit-scrollbar-thumb {
    background: color-mix(in srgb, var(--desktop-accent, #2563eb) 65%, #94a3b8);
    border-color: #e2e8f0;
  }

  /* Light mode: high contrast, solid backgrounds */
  .win-root.theme-light {
    color: #0f172a;
    background:
      radial-gradient(circle at 18% 12%, color-mix(in srgb, var(--desktop-accent) 18%, transparent), transparent 32%),
  ...
  .win-root.theme-light .desktop-shortcut:hover {
    background: #e2e8f0;
  }
  ...
  .win-root.theme-light .win-empty-note,
  .win-root.theme-light .win-window,
  .win-root.theme-light .taskbar,
  .win-root.theme-light .start-menu,
  .win-root.theme-light .search-panel {
    color: #0f172a;
    border-color: #cbd5e1;
    background: #ffffff;
  }

  .win-root.theme-light .win-titlebar {
    color: #0f172a;
    border-bottom-color: #d7dee8;
    background: #f8fafc;
  }

  .win-root.theme-light .window-actions button {
    color: #334155;
  }

  .win-root.theme-light .window-actions button:hover {
    background: #e2e8f0;
  }

  .win-root.theme-light .win-sidebar,
  .win-root.theme-light .win-content {
    background: #f1f5f9;
  }

  .win-root.theme-light .win-sidebar,
  .win-root.theme-light .panel-header,
  .win-root.theme-light .start-header,
  .win-root.theme-light .search-header,
  .win-root.theme-light .start-footer {
    border-color: #d7dee8;
  }

  .win-root.theme-light .user-card,
  .win-root.theme-light .content-hero,
  .win-root.theme-light .stat-card,
  .win-root.theme-light .win-panel,

  .win-root.theme-light .task-focus-switch {
    color: #334155;
    border-color: #d7dee8;
    background: #ffffff;
  }

  .win-root.theme-light .task-focus-switch:hover {
    background: #e2e8f0;
  }

  .win-root.theme-light .task-focus-switch.on {
    color: #ffffff;
    border-color: transparent;
    background: var(--desktop-accent);
  }

  .win-root.theme-light .task-focus-knob {
    background: #e2e8f0;
    box-shadow: inset 0 0 0 1px #cbd5e1;
  }

  .win-root.theme-light .task-focus-knob::after {
    background: #64748b;
  }

  .win-root.theme-light .task-focus-switch.on .task-focus-knob {
    background: rgba(255,255,255,.25);
    box-shadow: none;
  }

  .win-root.theme-light .task-focus-switch.on .task-focus-knob::after {
    background: #ffffff;
  }

  .win-root.theme-light .task-icon,
  .win-root.theme-light .task-search,
  .win-root.theme-light .table-row,
  .win-root.theme-light .hero-chip {
    color: #0f172a;
    border-color: #d7dee8;
    background: #ffffff;
  }

  .win-root.theme-light .task-icon:hover,
  .win-root.theme-light .task-icon.active,
  .win-root.theme-light .task-start:hover,
  .win-root.theme-light .side-item:hover,
  .win-root.theme-light .side-item.active,
  .win-root.theme-light .start-app:hover {
    background: #e2e8f0;
  }

  .win-root.theme-light .content-hero p,
  .win-root.theme-light .stat-card span,
  .win-root.theme-light .panel-header span,
  .win-root.theme-light .table-row span,
  .win-root.theme-light .user-card span,
  .win-root.theme-light .side-item span,
  .win-root.theme-light .task-right,
  .win-root.theme-light .task-search {
    color: #475569;
  }

  .win-root.theme-light .side-item {
    color: #0f172a;
  }

  .win-root.theme-light .score-pill {
    color: #166534;
    background: #dcfce7;
  }

  .win-root.theme-light .score-pill.negative {
    color: #991b1b;
    background: #fee2e2;
  }

  .win-root.theme-light .logout-button {
    color: #991b1b;
    border-color: #fecaca;
    background: #fff1f2;
  }

  .win-root.theme-light .task-app-menu {
    color: #0f172a;
    border-color: #cbd5e1;
    background: #ffffff;
  }

  .win-root.theme-light .task-app-menu button {
    color: #0f172a;
  }

  .win-root.theme-light .task-app-menu button:hover {
    background: #e2e8f0;
  }


  .win-root.theme-light .win-empty-note h1 {
    color: #0f172a;
  }

  .win-root.theme-light .win-empty-note p {
    color: #64748b;
  }

  .win-root.theme-light .task-icon.show-badge::before {
    box-shadow: 0 0 0 2px #ffffff;
  }



  .search-panel.align-left {
    left: 16px;
    transform: none;
    animation: menuUpLeft .18s ease both;
  }

  .start-menu.align-left {
    left: 16px;
    transform: none;
    animation: menuUpLeft .18s ease both;
  }

  @keyframes menuUpLeft {
    from { opacity: 0; transform: translateY(12px) scale(.98); }
    to { opacity: 1; transform: translateY(0) scale(1); }
  }

  .win-root.theme-light .start-app {
    color: #0f172a;
  }

  .win-root.theme-light .start-app span {
    color: #0f172a;
  }

  .win-root.theme-light .start-header h2,
  .win-root.theme-light .search-header h2 {
    color: #0f172a;
  }
  ...
  .win-root.theme-light .start-footer strong {
    color: #0f172a;
  }

  .win-root.theme-light .start-footer .user-card {
    color: #0f172a;
    background: transparent;
  }
  ...
  .win-root.theme-light .task-start {
    color: #ffffff;
  }
