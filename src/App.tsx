...
type AppKey = "dashboard" | "profile" | "messages" | "quickScore" | "ranking" | "contests" | "students" | "settings";
...
const DESKTOP_APPS: DesktopApp[] = [
  ...
  { key: "profile", title: "Profile", subtitle: "Hồ sơ học sinh", icon: UserRound, path: "/desktop/profile" },
  { key: "messages", title: "Messages", subtitle: "Tin nhắn và yêu cầu cấp quyền", icon: MessageCircle, path: "/desktop/messages" },
  { key: "quickScore", title: "Nhập điểm nhanh", subtitle: "Cộng/trừ điểm nề nếp và học tập", icon: ClipboardList, path: "/desktop/nhap-diem-nhanh" },
...
const DESKTOP_SHORTCUTS: AppKey[] = ["dashboard", "settings", "profile", "messages"];
...
function getDesktopAccent() {
  ...
  if (loginAccent?.toLowerCase() === "custom" && normalizeAccentValue(loginCustomAccent)) return normalizeAccentValue(loginCustomAccent) || ACCENT_COLORS.blue;
  const keys = ["login-accent", "login-accent-color", "accent-color", "accentColor", "accent", "desktop-accent", "desktop-accent-color", "theme-accent", "login-custom-accent", "custom-accent", "customAccent", "desktop-custom-accent"];
  for (const key of keys) {
...
function getAppFromPath(pathname: string): DesktopApp | null {
  return DESKTOP_APPS.find((app) => app.path === pathname) || null;
}
  ...
  return (
    <section className="win-content">
      ...
      <div className="panel-grid">
        <section className="win-panel"><div className="panel-header"><div><h2>Hoạt động gần đây</h2><span>Cập nhật điểm mới nhất</span></div><ChevronRight size={18} /></div><div className="table-like"><div className="table-row"><div><strong>Phát biểu xây dựng bài</strong><span>Học tập · vừa xong</span></div><div className="score-pill">+5</div></div><div className="table-row"><div><strong>Không đeo thẻ học sinh</strong><span>Nề nếp · 15 phút trước</span></div><div className="score-pill negative">-5</div></div><div className="table-row"><div><strong>Kiểm tra bài cũ đạt 10</strong><span>Học tập · hôm nay</span></div><div className="score-pill">+10</div></div></div></section>
        <section className="win-panel"><div className="panel-header"><div><h2>Lịch & thông báo</h2><span>Nhắc việc trong tuần</span></div><Bell size={18} /></div><div className="table-like"><div className="table-row"><div><strong>Tổng kết thi đua tuần</strong><span>Thứ 7 · 17:00</span></div><CalendarDays size={18} /></div><div className="table-row"><div><strong>Kiểm tra danh sách vi phạm</strong><span>Lớp trưởng / Bí thư</span></div><ShieldCheck size={18} /></div></div></section>
      </div>
    </section>
  );
}

function WindowsDesktop({ user, onLogout, initialPath }: { user: DesktopUser; onLogout: () => void; initialPath?: string }) {
  ...
  const bootPath = useRef(initialPath || window.location.pathname);
  ...
  useEffect(() => {
    const app = getAppFromPath(bootPath.current);
    if (app) window.setTimeout(() => openApp(app), 80);
    else if (bootPath.current.startsWith("/desktop/") && bootPath.current !== "/desktop") window.history.replaceState({}, "", "/desktop");
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("storage", update); window.removeEventListener("taskbar-settings-change", customUpdate); };
  }, []);

  useEffect(() => { localStorage.setItem("pinned-apps", JSON.stringify(pinnedApps)); }, [pinnedApps]);
  ...
  useEffect(() => {
    const updateIconsCovered = () => {
      ...
      setIconsCovered(visibleWindows.some((win) => { const rect = win.getBoundingClientRect(); return !(rect.right < iconRect.left || rect.left > iconRect.right || rect.bottom < iconRect.top || rect.top > iconRect.bottom); }));
    };
    updateIconsCovered();
  ...
  const openApp = (app: DesktopApp) => {
    ...
    setStartOpen(false); setSearchOpen(false); setTaskAppMenu(null);
    const exists = windows.some((item) => item.key === app.key);
    zCounter.current += 1;
    if (exists) setWindows((current) => current.map((item) => item.key === app.key ? { ...item, minimized: false, z: zCounter.current } : item));
    else { const offset = Math.min(windows.length * 28, 110); setWindows((current) => [...current, { key: app.key, x: offset, y: offset, z: zCounter.current, minimized: false, maximized: false }]); }
    setFocusedKey(app.key);
    window.history.pushState({}, "", app.path);
  };

  useEffect(() => {
    const handler = (event: Event) => { const detail = ((event as CustomEvent<ProfileRequest>).detail || {}) as ProfileRequest; setProfileRequest(detail); openApp(getApp("profile")); };
    window.addEventListener("a3k64-open-profile", handler);
    return () => window.removeEventListener("a3k64-open-profile", handler);
  }, [windows, user]);

  const toggleFocusMode = async () => { setStartOpen(false); setSearchOpen(false); setTaskAppMenu(null); try { if (!document.fullscreenElement) await document.documentElement.requestFullscreen?.(); else await document.exitFullscreen?.(); } catch { setFocusMode((value) => !value); } };
  ...
  const toggleMaximize = (appKey: AppKey) => { bringToFront(appKey, true); setWindows((current) => current.map((item) => item.key === appKey ? { ...item, maximized: !item.maximized } : item)); };
  const updateWindowPos = (appKey: AppKey, x: number, y: number) => setWindows((current) => current.map((item) => item.key === appKey ? { ...item, x, y } : item));

  const handleTitlebarMouseDown = (event: React.MouseEvent<HTMLElement>, win: WindowState) => {
    if (win.maximized || event.button !== 0) return;
    event.preventDefault(); bringToFront(win.key, true);
    const startX = event.clientX; const startY = event.clientY; const startPos = { x: win.x, y: win.y };
  ...
  const togglePinnedApp = (appKey: AppKey) => { setPinnedApps((current) => current.includes(appKey) ? current.filter((key) => key !== appKey) : [...current, appKey]); setTaskAppMenu(null); };
  const handleTaskbarAppClick = (appKey: AppKey) => { const win = windows.find((item) => item.key === appKey); if (win) { bringToFront(appKey, true); window.history.pushState({}, "", getApp(appKey).path); return; } openApp(getApp(appKey)); };
  ...
  return (
    <main className={`win-root theme-${resolvedTheme} ${focusMode ? "focus-mode" : ""}`} style={{ "--desktop-accent": accent } as React.CSSProperties}>
      ...
      <section className="win-desktop login-push-enter">
        <div ref={desktopIconsRef} className={`desktop-icons ${iconsCovered ? "covered" : ""}`}>
          {DESKTOP_APPS.filter((app) => DESKTOP_SHORTCUTS.includes(app.key)).map((app) => {
            const Icon = app.icon; const locked = !canOpenApp(app, user);
            return <button key={app.key} type="button" className="desktop-shortcut" draggable onDragStart={(event) => event.dataTransfer.setData("text/plain", app.key)} onDoubleClick={() => openApp(app)} title={locked ? "Bạn chưa có quyền xem mục này" : `${app.title} - bấm đúp để mở`}><div className="desktop-shortcut-icon">{locked ? <Lock /> : <Icon />}</div><span>{app.title}</span></button>;
          })}
        </div>
        ...
        {windows.map((win) => {
          const app = getApp(win.key); const ActiveIcon = app.icon; const isFocused = focusedKey === win.key;
          const fullBodyApp = app.key === "settings" || app.key === "dashboard" || app.key === "profile" || app.key === "messages";
          return (
            <section key={win.key} className={`win-window ${win.maximized ? "maximized" : ""} ${win.minimized ? "minimized" : ""} ${isFocused ? "focused" : ""}`} style={{ "--win-x": `${win.x}px`, "--win-y": `${win.y}px`, zIndex: win.z } as React.CSSProperties} onMouseDown={() => bringToFront(win.key, true)} onContextMenu={(event) => event.preventDefault()}>
              <header className="win-titlebar" onMouseDown={(event) => handleTitlebarMouseDown(event, win)} onDoubleClick={() => toggleMaximize(win.key)}>
                ...
                <div className="window-actions" onMouseDown={(event) => event.stopPropagation()}><button type="button" title="Thu nhỏ" onClick={() => minimizeWindow(win.key)}><Minus size={16} /></button><button type="button" title={win.maximized ? "Khôi phục" : "Phóng to"} onClick={() => toggleMaximize(win.key)}>{win.maximized ? <Minimize2 size={15} /> : <Maximize2 size={15} />}</button><button className="close" type="button" title="Đóng" onClick={() => closeWindow(win.key)}><X size={16} /></button></div>
              </header>
              <div className={`win-body ${fullBodyApp ? "settings-mode" : ""}`}>
                {!fullBodyApp && <aside className="win-sidebar"><div className="user-card"><div className="avatar">{user.photoURL ? <img src={user.photoURL} alt="Avatar" /> : getInitials(user.displayName)}</div><div style={{ minWidth: 0 }}><strong>{user.displayName || "Người dùng 12A3"}</strong><span>{user.email || user.role || "Đang đăng nhập"}</span></div></div><nav className="side-nav" aria-label="Ứng dụng">{DESKTOP_APPS.map((navApp) => { const Icon = navApp.icon; const locked = !canOpenApp(navApp, user); return <button key={navApp.key} type="button" className={`side-item ${app.key === navApp.key ? "active" : ""} ${locked ? "locked" : ""}`} style={{ "--app-accent": accent } as React.CSSProperties} onClick={() => openApp(navApp)}><div className="side-item-icon">{locked ? <Lock size={16} /> : <Icon size={16} />}</div><div style={{ minWidth: 0 }}><strong>{navApp.title}</strong><span>{navApp.subtitle}</span></div></button>; })}</nav><div className="side-bottom"><button type="button" className="logout-button" onClick={onLogout}><LogOut size={16} /> Đăng xuất</button></div></aside>}
                {app.key === "settings" ? <SettingsApp accent={accent} setAccent={setAccent} userEmail={user.email} userName={user.displayName} userPhotoURL={user.photoURL} /> : app.key === "dashboard" ? <ScoreboardApp userRole={user.role} /> : app.key === "profile" ? <ProfileApp userName={user.displayName} userEmail={user.email} requestedStudentId={profileRequest.studentId} requestedWeek={profileRequest.week} /> : app.key === "messages" ? <MessagesApp userName={user.displayName} userEmail={user.email} userRole={user.role} userGroup={user.group} /> : <GenericAppContent active={app} />}
              </div>
            </section>
          );
        })}
      ...
      <nav className={`taskbar align-${taskbarSettings.alignment} ${taskbarSettings.autoHide ? "auto-hide" : ""}`} aria-label="Windows 11 taskbar" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const appKey = event.dataTransfer.getData("text/plain") as AppKey; if (DESKTOP_APPS.some((app) => app.key === appKey) && !pinnedApps.includes(appKey)) setPinnedApps((current) => [...current, appKey]); }}>
        ...
        <div className="task-center"><button type="button" className="task-start" onClick={() => { setSearchOpen(false); setStartOpen((value) => !value); }} title="Start"><Menu size={19} /></button>{taskbarSettings.searchMode === "icon" ? <button type="button" className="task-icon" onClick={() => { setStartOpen(false); setSearchOpen((value) => !value); }} title="Tìm kiếm"><Search size={17} /></button> : <button type="button" className="task-search" onClick={() => { setStartOpen(false); setSearchOpen((value) => !value); }}><Search size={16} /><span>Tìm kiếm</span></button>}<button type="button" className={`task-focus-switch ${focusMode ? "on" : ""}`} onClick={toggleFocusMode} title={focusMode ? "Tắt tập trung toàn màn hình" : "Bật tập trung toàn màn hình"} aria-pressed={focusMode}><span className="task-focus-knob" /><span className="task-focus-text">{focusMode ? "ON" : "OFF"}</span></button>{taskbarSettings.taskView && <button type="button" className="task-icon" title="Chế độ xem tác vụ"><ListChecks size={17} /></button>}{taskbarSettings.widgets && <button type="button" className="task-icon" title="Tiện ích"><Sparkles size={17} /></button>}{taskbarSettings.resume && <button type="button" className="task-icon" title="Tiếp tục"><Home size={17} /></button>}{taskbarItems.map((appKey) => { const taskApp = getApp(appKey); const TaskIcon = taskApp.icon; const isOpen = windows.some((item) => item.key === appKey); const taskWindow = windows.find((item) => item.key === appKey); const isFocused = focusedKey === appKey && !taskWindow?.minimized; const isPinned = pinnedApps.includes(appKey); return <button key={appKey} type="button" className={`task-icon ${isFocused ? "active" : ""} ${isOpen ? "running-app" : ""} ${isPinned ? "pinned-app" : ""} ${taskbarSettings.badges && isOpen ? "show-badge" : ""}`} title={taskApp.title} onClick={() => handleTaskbarAppClick(appKey)} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setTaskAppMenu({ x: event.clientX, y: event.clientY, appKey }); }}><TaskIcon size={18} /></button>; })}</div>
...
export default function App() {
  ...
  const [initialPath, setInitialPath] = useState(typeof window !== "undefined" ? window.location.pathname : "/desktop");

  const handleLogin = (loginUser: DesktopUser) => {
    setLeavingLogin(true);
    const pathAtLogin = window.location.pathname;
    window.setTimeout(() => {
      setInitialPath(pathAtLogin);
      setUser({ ...loginUser, role: loginUser.role || "hoc_sinh" });
      if (!pathAtLogin.startsWith("/desktop/")) window.history.pushState({}, "", "/desktop");
      setLeavingLogin(false);
    }, 520);
  };

  const handleLogout = () => { window.history.pushState({}, "", "/"); setUser(null); };
