...
function get(k:string){return store.has(k)?store.get(k)!:null}
function put(k:string,v:string){store.set(k,String(v))}
function user(){return json<Session|null>(get(SESSION_KEY),null)?.user||null}
...
function applyTheme(v:unknown){const m=theme(v);if(!m)return;THEME.forEach(k=>put(k,m))}
function applyAccent(a:unknown,c?:unknown){const ak=accent(a)||accentFromColor(text(c));const col=color(c,ak);put("desktop-accent",col);put("desktop-accent-color",col);put("login-accent",ak);put("login-accent-color",col);put("login-custom-accent",col);put("accent-color",col);put("accent",ak)}
function apply(s:P){remote=true;try{if(s.theme)applyTheme(s.theme);if(s.accent||s.accentColor)applyAccent(s.accent,s.accentColor);if(s.desktopTransparency)put("desktop-transparency",s.desktopTransparency);if(s.accentTaskbar)put("accent-taskbar",s.accentTaskbar);if(s.accentBorders)put("accent-borders",s.accentBorders);if(s.taskbarSettings)put("taskbar-settings",JSON.stringify(s.taskbarSettings));if(s.pinnedApps)put("pinned-apps",JSON.stringify(s.pinnedApps));if(s.recentAccents)put("recent-accents",JSON.stringify(s.recentAccents))}finally{remote=false}fire()}
function loginSnap():P{const t=theme(get("login-theme")),a=accent(get("login-accent"));return{...(t?{theme:t}:{}),...(a?{accent:a,accentColor:color(get("login-accent-color"),a)}:{})}}
function accentState():P{const a=accent(get("login-accent"))||accent(get("accent"));const c=color(get("desktop-accent")||get("desktop-accent-color")||get("login-accent-color")||get("login-custom-accent"),a);return{accent:a||accentFromColor(c),accentColor:c}}
...
function queue(s:P|null){const a=account();if(!a||!s||remote)return;pending={...pending,...s};clearTimeout(timer);timer=window.setTimeout(()=>{const p=pending;pending={};void saveP(a,p).catch(e=>console.error("Không lưu được cá nhân hoá:",e))},250)}
...
Storage.prototype.getItem=function(k:string){if(this===localStorage)return get(k);return nativeGet.call(this,k)};
Storage.prototype.setItem=function(k:string,v:string){if(this!==localStorage)return nativeSet.call(this,k,v);put(k,v);if(k===SESSION_KEY){const a=account();if(a)void sync(a);return}queue(fromWrite(k,v))};
Storage.prototype.removeItem=function(k:string){if(this!==localStorage)return nativeRemove.call(this,k);store.delete(k);if(k===SESSION_KEY)lastAccount=""};
Storage.prototype.clear=function(){if(this!==localStorage)return nativeClear.call(this);store.clear();lastAccount=""};
