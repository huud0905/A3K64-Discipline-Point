...
function notifAdd(title, body) {
  const now  = new Date();
  const time = String(now.getHours()).padStart(2,'0') + ':' + String(now.getMinutes()).padStart(2,'0');
  notifList = [{ id:'n'+Date.now(), title:String(title||''), body:String(body||''), time, read:false }, ...notifList].slice(0, 50);
  _notifPatchBell();
  _notifPatchPanel();
}
function notifClearAll() { notifList = []; _notifPatchBell(); _notifPatchPanel(); }
function notifMarkAllRead() { notifList.forEach(function(n){n.read=true;}); _notifPatchBell(); }
function toggleNotifPanel() {
  notifPanelOpen = !notifPanelOpen;
  if (notifPanelOpen) notifMarkAllRead();
  _notifPatchBell();
  _notifPatchPanel();
}
function _notifUnread() { return notifList.filter(function(n){return !n.read;}).length; }
function _notifBellSVG() {
  return '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>';
}
function _escH(s) {
  return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
}
function _notifPatchBell() {
  var bell = document.getElementById('notif-bell-btn');
  if (!bell) return;
  var cnt = _notifUnread();
  bell.innerHTML = _notifBellSVG() + (cnt > 0 ? '<span class="notif-badge">' + (cnt > 9 ? '9+' : cnt) + '</span>' : '');
  bell.classList.toggle('has-unread', cnt > 0);
  bell.classList.toggle('panel-open', notifPanelOpen);
}
function _notifPatchPanel() {
  var panel = document.getElementById('notif-panel');
  if (!notifPanelOpen) { if (panel) panel.remove(); return; }
  if (!panel) {
  ...
  panel.innerHTML =
    '<div class="np-header">' +
      '<span class="np-title">Thông báo</span>' +
      (items.length ? '<button class="np-clear-all" onclick="notifClearAll()">Xoá tất cả</button>' : '') +
    '</div>' +
    '<div class="np-body">' +
      (items.length === 0
        ? '<div class="np-empty"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="opacity:.3"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg><span>Không có thông báo</span></div>'
        : items.map(function(n) {
            return '<div class="np-item' + (n.read ? '' : ' unread') + '">' +
              '<div class="np-item-header">' +
                '<strong class="np-item-app">' + _escH(n.title) + '</strong>' +
                '<span class="np-item-time">' + _escH(n.time) + '</span>' +
              '</div>' +
              '<p class="np-item-body">' + _escH(n.body) + '</p>' +
            '</div>';
          }).join('')) +
    '</div>';
}
...
function bringToFront(key, restore = true) {
  ...
  // Click để focus 1 cửa sổ ĐÃ mở trước đây phải đóng Start Menu/Search/menu
  // ngữ cảnh còn sót lại, giống hệt hành vi khi mở app mới qua openApp().
  let overlayChanged = false;
  if (startOpen) { startOpen = false; overlayChanged = true; }
  if (searchOpen) { searchOpen = false; overlayChanged = true; }
  if (taskAppMenu) { taskAppMenu = null; overlayChanged = true; }
  if (overlayChanged) {
    document.querySelectorAll('.start-menu, .search-panel, .task-app-menu').forEach(el => el.remove());
  }
...
function openApp(app, e) {
  ...
  // Dọn sạch overlay ngay trong DOM (không chỉ dựa vào cờ startOpen/searchOpen),
  // để tránh trường hợp Start Menu / Search Panel còn sót lại đè lên cửa sổ vừa mở.
  document.querySelectorAll('.start-menu, .search-panel, .task-app-menu').forEach(el => el.remove());
  zCounter++;
...
/* ---------- Drag ---------- */
function getDisplayScaleRatio() {
  const n = Number(localStorage.getItem('a3k64-display-scale'));
  return (n > 0 ? n : 100) / 100;
}

// Kích thước nội bộ (trước-scale) của #desktop-root — dùng cho snap zone
// placement thay vì 100vw/100vh (viewport thật, không bị scale).
function getRootInternalSize() {
  const ratio = getDisplayScaleRatio();
  return { w: window.innerWidth / ratio, h: window.innerHeight / ratio };
}
...
function startDrag(e, win) {
  ...
  function move(mv) {
    ...
    // #desktop-root có thể đang bị transform:scale(ratio) (Cài đặt → Màn
    // hình → Tỷ lệ và bố cục). Toạ độ x/y của cửa sổ là toạ độ NỘI BỘ
    // (trước-scale) của root, còn e.clientX/clientY luôn là toạ độ MÀN
    // HÌNH THẬT — nếu cộng thẳng delta màn hình vào toạ độ nội bộ, cửa sổ
    // sẽ trôi nhanh/chậm hơn con trỏ chuột. Chia delta cho ratio để quy
    // đổi về đúng không gian toạ độ nội bộ của root.
    const ratio = getDisplayScaleRatio();
    const rawX = startPos.x + (mv.clientX - startX) / ratio;
    const rawY = startPos.y + (mv.clientY - startY) / ratio;

    // Biên giới hạn cũng phải quy đổi: window.innerWidth/innerHeight là
    // kích thước màn hình thật, còn winW/winH (offsetWidth/offsetHeight)
    // đã được trình duyệt tính theo hệ toạ độ nội bộ (trước-scale) —
    // nên chia đôi bên viewport cho ratio để so cùng đơn vị với winW/winH.
    const vw = window.innerWidth  / ratio;
    const vh = window.innerHeight / ratio;
    const clampedX = Math.max(-(winW - MIN_VISIBLE_X), Math.min(rawX, vw - MIN_VISIBLE_X));
    const clampedY = Math.max(-TITLEBAR_H + 8,          Math.min(rawY, vh - TASKBAR_H - TITLEBAR_H));
...
function startResize(e, key, dir) {
  ...
  function move(mv) {
    const ratio = getDisplayScaleRatio();
    const dX = (mv.clientX - startX) / ratio;
    const dY = (mv.clientY - startY) / ratio;
...
function buildWindowContent(app) {
  ...
  // "Trực nhật" mở app Duty thật (HTML/CSS/JS thuần), nhúng qua iframe
  // cùng-origin, cùng quy ước với dashboard/settings/students ở trên.
  if (app.key === 'duty') {
    return `
      <div class="win-embed">
        <iframe class="win-embed-frame" src="../modules/duty/duty-window.html" title="${app.title}" loading="lazy"></iframe>
      </div>`;
  }
...
function buildWindowHTML(win) {
  ...
  // Snap zone placement dùng px nội bộ (trước-scale) thay vì vw/vh (viewport
  // thật). Khi display scale ≠ 100%, vw/vh không khớp với không gian trong
  // root → window snapped bị sai vị trí và kích thước.
  const _rsz = win.snapZone ? getRootInternalSize() : null;
  const TASKBAR_TOTAL = 58; // taskbar height (48) + bottom gap (8) + border ≈ 58px nội bộ
  const sizeStyle = win.maximized
    ? ''
    : win.snapZone
      ? `;position:absolute;transform:none;min-height:0;` +
        `left:${(_rsz.w * win.snapZone.x / 100).toFixed(2)}px;` +
        `top:${((_rsz.h - TASKBAR_TOTAL) * win.snapZone.y / 100).toFixed(2)}px;` +
        `width:${(_rsz.w * win.snapZone.w / 100).toFixed(2)}px;` +
        `height:${((_rsz.h - TASKBAR_TOTAL) * win.snapZone.h / 100).toFixed(2)}px`
  ...
  return `
    <nav class="taskbar align-${alignment} ${taskbarSettings.autoHide ? 'auto-hide' : ''}" aria-label="Taskbar">
      ...
      <div class="task-right">
        <div class="task-clock-wrap" onclick="toggleNotifPanel()">
          <span id="clock-time"></span>
          <span id="clock-date"></span>
        </div>
        <button id="notif-bell-btn" class="task-notif-bell" onclick="toggleNotifPanel()" title="Thông báo">
          ${_notifBellSVG()}${_notifUnread()>0?`<span class="notif-badge">${_notifUnread()>9?'9+':_notifUnread()}</span>`:''}
        </button>
      </div>
    </nav>`;
}
...
function renderWindows() {
  ...
  windows.forEach(win => {
    ...
    if (win.maximized) {
      ...
      // Dùng px nội bộ thay vì vw/vh (xem comment tại sizeStyle phía trên).
      const z = win.snapZone;
      const rs = getRootInternalSize();
      const TB = 58;
      el.style.position   = 'absolute';
      el.style.transform  = 'none';
      el.style.minHeight  = '0';
      el.style.left       = (rs.w * z.x / 100).toFixed(2) + 'px';
      el.style.top        = ((rs.h - TB) * z.y / 100).toFixed(2) + 'px';
      el.style.width      = (rs.w * z.w / 100).toFixed(2) + 'px';
      el.style.height     = ((rs.h - TB) * z.h / 100).toFixed(2) + 'px';
    } else {
      el.style.removeProperty('position');
      el.style.removeProperty('transform');
      el.style.removeProperty('min-height');
...
function _renderTaskViewPeek(anchorBtn) {
  ...
  // getBoundingClientRect() trả toạ độ màn hình thật → chia ratio để về
  // không gian nội bộ của #desktop-root (position:absolute).
  const ratio = getDisplayScaleRatio();
  const rLeft  = rect.left  / ratio;
  const rTop   = rect.top   / ratio;
  const rWidth = rect.width / ratio;
  const rH     = window.innerHeight / ratio;
  const thumbW = 160, gap = 10;
  const totalW = taskViewDesktops.length * (thumbW + gap) - gap;
  let left = rLeft + rWidth / 2 - totalW / 2;
  left = Math.max(8, Math.min(left, rH - totalW - 8));
  peek.style.cssText = `left:${left}px;bottom:${rH - rTop + 8}px`;
...
function showTaskAppMenu(e, appKey) {
  ...
  // clientX/clientY ở toạ độ màn hình thật; chia ratio để về không gian
  // nội bộ của #desktop-root (sau khi applyDisplayScale scale root).
  const ratio = getDisplayScaleRatio();
  taskAppMenu = { x: e.clientX / ratio, y: e.clientY / ratio, appKey };
...
// Click bên TRONG một iframe app (Bảng điểm A3, Trực nhật, Sơ đồ lớp...) không
// bubble ra tới document của trang cha, nên listener 'click' phía trên sẽ
// không bao giờ chạy khi thao tác trong app lúc Start Menu/Search đang mở.
// Khi focus chuyển vào iframe, window cha luôn nhận sự kiện 'blur' — dùng nó
// làm tín hiệu thay thế để đóng các overlay còn sót lại.
window.addEventListener('blur', () => {
  setTimeout(() => {
    if (document.activeElement && document.activeElement.tagName === 'IFRAME') {
      let changed = false;
      if (startOpen) { startOpen = false; changed = true; }
      if (searchOpen) { searchOpen = false; changed = true; }
      if (taskAppMenu) { taskAppMenu = null; changed = true; }
      if (changed) _patchOverlays();
    }
  }, 0);
});
...
window.addEventListener('storage', (e) => {
  const RELEVANT_KEYS = ['login-accent','accent-color','accent','desktop-accent','a3k64-accent','a3k64-theme','login-theme','desktop-theme','theme-mode','taskbar-settings','a3k64-display-scale'];
...
function applyDisplayScale() {
  ...
  if (ratio === 1) {
    ...
    // Khi không scale, --root-w/h = kích thước viewport thật
    root.style.setProperty('--root-w', window.innerWidth  + 'px');
    root.style.setProperty('--root-h', window.innerHeight + 'px');
  } else {
    // Kích thước nội bộ (trước-scale) của root — lớn hơn viewport để sau khi
    // scale(ratio) vừa khít màn hình. Dùng làm --root-w/h thay cho 100vw/100vh
    // trong CSS của taskbar, maximized window và snap zones, vì 100vw/100vh
    // luôn bám vào viewport thật (không bị ảnh hưởng bởi transform:scale).
...
// px ở trên tính từ kích thước viewport tại thời điểm áp — phải tính lại
// mỗi khi cửa sổ trình duyệt đổi kích thước.
...
function initDesktop(desktopUser) {
  ...
  /* ── Preload dữ liệu nền (scoreboard, seating, rules) ── */
  _initPreload(desktopUser);
...
function _initPreload(desktopUser) {
  ...
  function _runPreload() {
    if (window.A3K64Preload) {
      A3K64Preload.run(gasUrl, desktopUser);
      return;
    }
    /* preload.js chưa có — inject rồi chạy */
    const s = document.createElement('script');
    s.src = '../assets/js/preload.js';
    s.onload = function () {
      if (window.A3K64Preload) A3K64Preload.run(gasUrl, desktopUser);
    };
    s.onerror = function () {
      console.warn('[Desktop] Không tải được preload.js.');
    };
    document.head.appendChild(s);
  }

  /* Chờ render xong một frame rồi mới fetch, tránh block UI */
  requestAnimationFrame(function () { setTimeout(_runPreload, 0); });
}

function _initNotify() {
  const gasUrl = window.A3K64_CONFIG?.gasUrl || null;

  function _startWithNotify() {
    if (!window.A3Notify) return;
    /* Cho A3Notify biết gasUrl để broadcast() có thể push lên GAS */
    A3Notify.gasUrl = gasUrl;
    /* Bắt đầu poll GAS nhận broadcast từ user/device khác */
    if (gasUrl) A3Notify.startBroadcastPoll(gasUrl, 12000);
  }
  ...
  /* notify.js chưa load — inject script rồi callback */
  const s = document.createElement('script');
  /* Đường dẫn tương đối từ desktop/ lên assets/js/ */
  s.src = '../assets/js/notify.js';
  s.onload = _startWithNotify;
  s.onerror = function() {
    console.warn('[Desktop] Không tải được notify.js — broadcast sẽ không hoạt động.');
  };
  document.head.appendChild(s);
}
  ...
  const zonesHTML = snapAssist.remainingZoneIdxs.map(idx => {
    ...
    return `
      <div class="snap-assist-zone" style="
        left:${(_rs.w * z.x / 100).toFixed(2)}px;
        top:${((_rs.h - _TB) * z.y / 100).toFixed(2)}px;
        width:${(_rs.w * z.w / 100).toFixed(2)}px;
        height:${((_rs.h - _TB) * z.h / 100).toFixed(2)}px;
      ">
...
function positionSnapPopover(el, anchorEl) {
  ...
  const margin = 8;
  // getBoundingClientRect() trả toạ độ màn hình thật → chia ratio để về
  // không gian nội bộ của #desktop-root (snap-popover dùng position:absolute).
  const ratio  = getDisplayScaleRatio();
  const rLeft   = rect.left   / ratio;
  const rBottom = rect.bottom / ratio;
  const rWidth  = rect.width  / ratio;
  const rVW     = window.innerWidth / ratio;

  let left = rLeft + rWidth / 2 - popW / 2;
  left = Math.max(margin, Math.min(left, rVW - popW - margin));

  el.style.position  = 'absolute';
  el.style.top        = (rBottom + 8) + 'px';
  ...
  const arrowLeft = rLeft + rWidth / 2 - left;
...
/* ══════════════════════════════════════════════════════
   NOTIFICATION BRIDGE — nhận postMessage từ iframe modules
   scoreboard, seating... gọi: window.parent.postMessage({type:'a3k64-notif', title, body}, '*')
══════════════════════════════════════════════════════ */
window.addEventListener('message', function(e) {
  if (!e.data || e.data.type !== 'a3k64-notif') return;
