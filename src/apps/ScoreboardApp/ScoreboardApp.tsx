...
export default function ScoreboardApp({ userRole, userGroup }: ScoreboardAppProps) {
  ...
  const permissionNote = isGroupLeader ? `Tổ trưởng chỉ được sửa học sinh thuộc tổ ${userGroupNumber || "của mình"}.` : "";
  ...
  const renderOverviewContent = () => { if (viewMode === "students") return <StudentTable title="Danh sách cá nhân" students={summaries} compact onOpenStudent={canUseScoringTab ? openStudent : undefined} canEditStudent={overviewPermissionCheck} />; return <OverviewGroupsPage summaries={groupFilteredRawSummaries} podiumSummaries={rawSummaries} week={week} onOpenStudent={canUseScoringTab ? openStudent : () => undefined} canEditStudent={overviewPermissionCheck} />; };
