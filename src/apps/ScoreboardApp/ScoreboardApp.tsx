...
export default function ScoreboardApp({ userRole, userGroup }: ScoreboardAppProps) {
  ...
  const weekLockMessage = isCurrentWeekLockedForGroupLeader
    ? `Tuần ${week} đã khóa chấm điểm${currentWeekSetting?.start || currentWeekSetting?.end ? ` (${currentWeekSetting?.start || "?"} → ${currentWeekSetting?.end || "?"})` : ""}.`
    : "";
  const canUseScoringTab = hasFullAccess || (isGroupLeader && !isCurrentWeekLockedForGroupLeader);
