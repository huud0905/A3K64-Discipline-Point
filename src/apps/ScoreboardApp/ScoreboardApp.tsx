...
function normalizeUserGroup(value?: number | string | null) { const parsed = Number(String(value ?? "").replace(/[^0-9]/g, "")); return parsed === 1 || parsed === 2 || parsed === 3 || parsed === 4 ? parsed : null; }
...
export default function ScoreboardApp({ userRole, userGroup }: ScoreboardAppProps) {
  ...
  const userGroupNumber = normalizeUserGroup(userGroup);
  ...
  const canEditStudent = useCallback((student: StudentScoreSummary) => {
    ...
    if (isGroupLeader) return Boolean(userGroupNumber && Number(student.group) === userGroupNumber);
    return false;
  }, [hasFullAccess, isGroupLeader, userGroupNumber]);
  ...
  const applyRemoteData = useCallback((remoteData: NonNullable<Awaited<ReturnType<typeof fetchScoreboardFromGas>>>, options?: { silent?: boolean; notify?: boolean }) => {
    ...
    const now = Date.now();
    const newEvents = nextEvents.filter((event) => { const signature = eventSignature(event); if (seenBefore.has(signature)) return false; const time = eventTime(event); if (!time) return false; return time >= liveStartedAtRef.current - FRESH_EVENT_MARGIN_MS && time <= now + FRESH_EVENT_MARGIN_MS; }).sort((a, b) => eventTime(b) - eventTime(a));
    ...
    setStudents(nextStudents); setEvents(nextEvents); setWeeks(nextWeeks); setWeek(nextWeekValue); setDataSource("gas");
    ...
    if (options?.notify && initializedLiveRef.current && newEvents.length) {
      const event = newEvents[0]; const studentName = studentMap.get(event.studentId) || "học sinh"; const actor = event.createdBy || "Có người"; const points = event.points || 0; const kind: LiveToastKind = isScoreboardForeground(scoreboardRootRef.current) ? "foreground" : "background"; const pointsText = points > 0 ? `+${points}` : String(points);
  ...
  const deleteScore = (eventId: string) => {
    if (!canEditCurrentStudent) return;
    setEvents((current) => current.filter((event) => event.id !== eventId)); const event = eventsRef.current.find((item) => item.id === eventId); if (event) seenSignaturesRef.current.delete(eventSignature(event));
    if (dataSource === "gas") { void deleteScoreEventInGas(eventId).catch(() => { setSyncMessage("Không xoá được trên Google Sheets. Hãy bấm làm mới để kiểm tra lại."); void loadScoreboardData(); }); }
  };
  ...
  const addScore = (event: Omit<ScoreEvent, "id"> | Omit<ScoreEvent, "id" | "createdAt">) => {
    ...
    if (!targetStudent || !canEditStudent(targetStudent)) { setSyncMessage("Bạn không có quyền sửa điểm học sinh này."); window.setTimeout(() => setSyncMessage(""), 2600); return; }
    const eventData = event as Omit<ScoreEvent, "id"> & { createdAt?: string }; const temporaryEvent: ScoreEvent = { ...eventData, id: `local-${Date.now()}-${Math.random().toString(36).slice(2)}`, createdAt: eventData.createdAt || new Date().toISOString() };
    seenSignaturesRef.current.add(eventSignature(temporaryEvent)); setEvents((current) => [temporaryEvent, ...current]);
    if (dataSource === "gas") { void createScoreEventInGas(temporaryEvent).then((savedEvent) => { seenSignaturesRef.current.add(eventSignature(savedEvent)); setEvents((current) => current.map((item) => (item.id === temporaryEvent.id ? savedEvent : item))); }).catch(() => setSyncMessage("Không lưu được lên Google Sheets. Dữ liệu đang tạm nằm trên trình duyệt.")); }
  };
  ...
  return (
    <div className={`scoreboard-app role-${normalizedRole} ${isStudentOnly ? "student-readonly-mode" : ""}`} ref={scoreboardRootRef}>
      <aside className="scoreboard-left-tools">
        ...
        {permissionNote && <div className="score-permission-banner">{permissionNote}</div>}
