...
import { createScoreEventInGas, createWeekInGas, deleteScoreEventInGas, fetchScoreboardFromGas, WeekSetting } from "../../lib/gasApi";
...
function readSavedSessionUser() { try { const session = JSON.parse(localStorage.getItem(SESSION_KEY) || "null") as { user?: { group?: unknown; to?: unknown; displayName?: unknown; hoten?: unknown; name?: unknown; email?: unknown } } | null; return session?.user || null; } catch { return null; } }
...
export default function ScoreboardApp({ userRole, userGroup }: ScoreboardAppProps) {
  ...
  const deletedScoreIdsRef = useRef<Set<string>>(new Set());
  const optimisticEventsRef = useRef<Map<string, ScoreEvent>>(new Map());
  ...
  const weekLockMessage = isCurrentWeekLockedForGroupLeader
    ? `Tuần ${week} đã quá hạn chấm điểm${currentWeekSetting?.start || currentWeekSetting?.end ? ` (${currentWeekSetting?.start || "?"} → ${currentWeekSetting?.end || "?"})` : ""}. Chỉ khóa nút Sửa, vẫn có thể xem bảng chấm.`
    : "";
  ...
  const canEditStudent = useCallback((student: StudentScoreSummary) => { if (hasFullAccess) return true; if (isGroupLeader) return !isCurrentWeekLockedForGroupLeader && Boolean(userGroupNumber && Number(student.group) === userGroupNumber); return false; }, [hasFullAccess, isGroupLeader, isCurrentWeekLockedForGroupLeader, userGroupNumber]);
  const applyLocalScoreState = useCallback((incomingEvents: ScoreEvent[]) => { const deleted = deletedScoreIdsRef.current; const optimistic = optimisticEventsRef.current; const nextEvents = incomingEvents.filter((event) => !deleted.has(event.id)); const signatures = new Set(nextEvents.map(eventSignature)); optimistic.forEach((event) => { if (deleted.has(event.id)) return; const signature = eventSignature(event); if (signatures.has(signature)) return; nextEvents.unshift(event); signatures.add(signature); }); return nextEvents; }, []);

  const showLiveToast = useCallback((toast: Omit<LiveToast, "id">) => { const id = `toast-${Date.now()}-${Math.random().toString(36).slice(2)}`; setLiveToasts((current) => [{ ...toast, id }, ...current].slice(0, 4)); window.setTimeout(() => setLiveToasts((current) => current.filter((item) => item.id !== id)), toast.kind === "foreground" ? 3600 : 5200); }, []);
  const applyRemoteData = useCallback((remoteData: NonNullable<Awaited<ReturnType<typeof fetchScoreboardFromGas>>>, options?: { silent?: boolean; notify?: boolean }) => { const nextStudents = remoteData.students; const nextEvents = applyLocalScoreState(remoteData.events); const nextWeeks = remoteData.weeks.length ? remoteData.weeks : [weekRef.current || 1]; const seenBefore = seenSignaturesRef.current; const remoteSignatures = new Set(nextEvents.map(eventSignature)); const now = Date.now(); const newEvents = nextEvents.filter((event) => { const signature = eventSignature(event); if (seenBefore.has(signature)) return false; const time = eventTime(event); if (!time) return false; return time >= liveStartedAtRef.current - FRESH_EVENT_MARGIN_MS && time <= now + FRESH_EVENT_MARGIN_MS; }).sort((a, b) => eventTime(b) - eventTime(a)); const studentMap = new Map(nextStudents.map((student) => [student.id, student.name])); const nextWeekValue = nextWeeks.includes(weekRef.current) ? weekRef.current : nextWeeks[0] || 1; setStudents(nextStudents); setEvents(nextEvents); setWeeks(nextWeeks); setWeekSettings(remoteData.weekSettings || []); setWeek(nextWeekValue); setDataSource("gas"); if (!options?.silent) setSyncMessage(!nextStudents.length ? "Không đọc được học sinh trong sheet TUẦN hiện tại." : ""); if (options?.notify && initializedLiveRef.current && newEvents.length) { const event = newEvents[0]; const studentName = studentMap.get(event.studentId) || "học sinh"; const actor = event.createdBy || "Có người"; const points = event.points || 0; const kind: LiveToastKind = isScoreboardForeground(scoreboardRootRef.current) ? "foreground" : "background"; const pointsText = points > 0 ? `+${points}` : String(points); showLiveToast({ kind, title: kind === "foreground" ? "Điểm vừa được cập nhật" : "Bảng điểm A3 vừa cập nhật", message: `${actor} vừa sửa điểm của ${studentName}: ${event.title} (${pointsText})`, points }); } seenSignaturesRef.current = remoteSignatures; initializedLiveRef.current = true; }, [applyLocalScoreState, showLiveToast]);
  const loadScoreboardData = useCallback(async () => { setDataSource("loading"); setSyncMessage(""); const remoteData = await fetchScoreboardFromGas(); if (!remoteData) { const localStudents = mockStudents; const localEvents = readLocalEvents(); const localWeeks = readLocalWeeks(); setStudents(localStudents); setEvents(localEvents); setWeeks(localWeeks); setWeekSettings([]); setDataSource("local"); setSyncMessage("Đang dùng dữ liệu cục bộ. Chưa cấu hình hoặc chưa đọc được Google Apps Script."); seenSignaturesRef.current = new Set(localEvents.map(eventSignature)); liveStartedAtRef.current = Date.now(); initializedLiveRef.current = true; return; } applyRemoteData(remoteData, { silent: false, notify: false }); liveStartedAtRef.current = Date.now(); }, [applyRemoteData]);
  ...
  useEffect(() => { const timer = window.setInterval(async () => { if (pollingRef.current || dataSource !== "gas") return; pollingRef.current = true; try { const remoteData = await fetchScoreboardFromGas(); if (remoteData) applyRemoteData(remoteData, { silent: true, notify: true }); } finally { pollingRef.current = false; } }, LIVE_REFRESH_MS); return () => window.clearInterval(timer); }, [applyRemoteData, dataSource]);

  const isOverviewMode = viewMode === "overview"; const isScoringTab = activeTab === "scoring"; const shownStatusFilter = isOverviewMode || isScoringTab ? "all" : statusFilter; const shownSortMode = isOverviewMode || isScoringTab ? "score-desc" : sortMode;
  ...
  const summaries = useMemo(() => [...filteredSummaries].sort((a, b) => { const nameCompare = compareByGivenName(a, b); if (shownSortMode === "name-az") return nameCompare; if (shownSortMode === "name-za") return -nameCompare; if (shownSortMode === "score-asc") return a.total - b.total || nameCompare; return b.total - a.total || nameCompare; }), [filteredSummaries, shownSortMode]);
  const scoringSummaries = useMemo(() => { const byId = new Map(groupFilteredRawSummaries.map((student) => [student.id, student])); return students.map((student) => byId.get(student.id)).filter((student): student is NonNullable<typeof student> => Boolean(student)); }, [groupFilteredRawSummaries, students]);
  const groupStats = useMemo(() => getGroupStats(groupFilteredRawSummaries), [groupFilteredRawSummaries]); const totalScore = groupFilteredRawSummaries.reduce((sum, student) => sum + student.total, 0); const goodCount = groupFilteredRawSummaries.filter((student) => student.status === "Tốt" || student.status === "Khá").length; const warningCount = groupFilteredRawSummaries.filter((student) => student.status === "Chưa đạt").length; const topGroup = [...groupStats].sort((a, b) => b.average - a.average || b.total - a.total)[0]; const editingStudent = rawSummaries.find((student) => student.id === editingStudentId) || null; const canEditCurrentStudent = editingStudent ? canEditStudent(editingStudent) : false;
  const openStudent = (studentId: string) => { const student = rawSummaries.find((item) => item.id === studentId); if (student && canEditStudent(student)) setEditingStudentId(studentId); }; const editStudent = (studentId: string) => openStudent(studentId);
  const deleteScore = (eventId: string) => { if (!canEditCurrentStudent) return; deletedScoreIdsRef.current.add(eventId); optimisticEventsRef.current.delete(eventId); setEvents((current) => current.filter((event) => event.id !== eventId)); const event = eventsRef.current.find((item) => item.id === eventId); if (event) seenSignaturesRef.current.delete(eventSignature(event)); if (dataSource === "gas") { void deleteScoreEventInGas(eventId).catch((error) => { setSyncMessage(error instanceof Error ? error.message : "Không xoá được trên Google Sheets. Hãy bấm làm mới để kiểm tra lại."); }); } };
  const addScore = (event: Omit<ScoreEvent, "id"> | Omit<ScoreEvent, "id" | "createdAt">) => { const targetStudent = rawSummaries.find((student) => student.id === event.studentId); if (!targetStudent || !canEditStudent(targetStudent)) { setSyncMessage(weekLockMessage || "Bạn không có quyền sửa điểm học sinh này."); window.setTimeout(() => setSyncMessage(""), 3200); return; } const eventData = event as Omit<ScoreEvent, "id"> & { createdAt?: string }; const temporaryEvent: ScoreEvent = { ...eventData, id: `local-${Date.now()}-${Math.random().toString(36).slice(2)}`, createdAt: eventData.createdAt || new Date().toISOString() }; optimisticEventsRef.current.set(temporaryEvent.id, temporaryEvent); seenSignaturesRef.current.add(eventSignature(temporaryEvent)); setEvents((current) => [temporaryEvent, ...current]); if (dataSource === "gas") { void createScoreEventInGas(temporaryEvent).then((savedEvent) => { optimisticEventsRef.current.delete(temporaryEvent.id); optimisticEventsRef.current.set(savedEvent.id, savedEvent); seenSignaturesRef.current.add(eventSignature(savedEvent)); setEvents((current) => current.map((item) => (item.id === temporaryEvent.id ? savedEvent : item))); }).catch((error) => { setSyncMessage(error instanceof Error ? error.message : "Không lưu được lên Google Sheets. Dữ liệu đang tạm nằm trên trình duyệt."); }); } };
  const saveScoreChanges = useCallback((changes: ScoreChanges) => { const deletionSet = new Set(changes.deletions); const now = Date.now(); const allowedAdditions = changes.additions.filter((event) => { const targetStudent = rawSummaries.find((student) => student.id === event.studentId); return targetStudent && canEditStudent(targetStudent); }); const temporaryEvents: ScoreEvent[] = allowedAdditions.map((event, index) => ({ ...event, id: `local-${now}-${index}-${Math.random().toString(36).slice(2)}`, createdAt: event.createdAt || new Date().toISOString() })); if (!temporaryEvents.length && !deletionSet.size) return; temporaryEvents.forEach((event) => { optimisticEventsRef.current.set(event.id, event); seenSignaturesRef.current.add(eventSignature(event)); }); deletionSet.forEach((eventId) => { deletedScoreIdsRef.current.add(eventId); optimisticEventsRef.current.delete(eventId); const removed = eventsRef.current.find((event) => event.id === eventId); if (removed) seenSignaturesRef.current.delete(eventSignature(removed)); }); setEvents((current) => [...temporaryEvents, ...current.filter((event) => !deletionSet.has(event.id))]); setSyncMessage(""); if (dataSource === "gas") { temporaryEvents.forEach((temporaryEvent) => { void createScoreEventInGas(temporaryEvent).then((savedEvent) => { optimisticEventsRef.current.delete(temporaryEvent.id); optimisticEventsRef.current.set(savedEvent.id, savedEvent); seenSignaturesRef.current.add(eventSignature(savedEvent)); setEvents((current) => current.map((item) => (item.id === temporaryEvent.id ? savedEvent : item))); }).catch((error) => { setSyncMessage(error instanceof Error ? error.message : "Không lưu được lên Google Sheets. Dữ liệu đang tạm nằm trên trình duyệt."); }); }); changes.deletions.forEach((eventId) => { void deleteScoreEventInGas(eventId).catch((error) => { setSyncMessage(error instanceof Error ? error.message : "Không xoá được trên Google Sheets. Hãy bấm làm mới để kiểm tra lại."); }); }); } }, [canEditStudent, dataSource, rawSummaries]);
  const createNewWeek = async () => { if (!canCreateWeek || isCreatingWeek) return; setCreateWeekConfirmOpen(false); setIsCreatingWeek(true); try { if (dataSource === "gas") { await createWeekInGas(nextWeek); await loadScoreboardData(); } else { setWeeks((current) => Array.from(new Set([...current, nextWeek])).sort((a, b) => a - b)); } setWeek(nextWeek); setActiveTab("scoring"); setSyncMessage(""); } catch { setSyncMessage(`Không tạo được tuần ${nextWeek} trên Google Sheets.`); } finally { setIsCreatingWeek(false); } };
  const requestCreateWeek = () => { if (!canCreateWeek || isCreatingWeek) return; setCreateWeekConfirmOpen(true); };
  const resetLocalData = () => { deletedScoreIdsRef.current.clear(); optimisticEventsRef.current.clear(); localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(WEEK_STORAGE_KEY); if (dataSource === "gas") { void loadScoreboardData(); return; } setStudents(mockStudents); setEvents(mockScoreEvents); setWeeks(SCORE_WEEKS); setWeek(1); setWeekSettings([]); seenSignaturesRef.current = new Set(mockScoreEvents.map(eventSignature)); liveStartedAtRef.current = Date.now(); };
  const overviewPermissionCheck = canUseScoringTab ? canEditStudent : undefined;
  const renderOverviewContent = () => { if (viewMode === "students") return <StudentTable title="Danh sách cá nhân" students={summaries} compact onOpenStudent={canUseScoringTab ? openStudent : undefined} canEditStudent={overviewPermissionCheck} highlightStudentName={highlightStudentName} />; return <OverviewGroupsPage summaries={groupFilteredRawSummaries} podiumSummaries={rawSummaries} week={week} onOpenStudent={canUseScoringTab ? openStudent : () => undefined} canEditStudent={overviewPermissionCheck} highlightStudentName={highlightStudentName} />; };
