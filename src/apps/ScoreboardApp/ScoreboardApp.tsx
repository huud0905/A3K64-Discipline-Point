...
function readSavedUserGroup() { try { const session = JSON.parse(localStorage.getItem(SESSION_KEY) || "null") as { user?: { group?: unknown; to?: unknown } } | null; return parseGroup(session?.user?.group as string | number | null) || parseGroup(session?.user?.to as string | number | null); } catch { return null; } }
...
export default function ScoreboardApp({ userRole, userGroup }: ScoreboardAppProps) {
  ...
  const permissionNote = isStudentOnly ? "Tài khoản học sinh chỉ được xem tổng quan." : isGroupLeader ? `Tổ trưởng chỉ được sửa học sinh thuộc tổ ${userGroupNumber || "của mình"}.` : "";
  const canEditStudent = useCallback((student: StudentScoreSummary) => { if (hasFullAccess) return true; if (isGroupLeader) return Boolean(userGroupNumber && Number(student.group) === userGroupNumber); return false; }, [hasFullAccess, isGroupLeader, userGroupNumber]);
