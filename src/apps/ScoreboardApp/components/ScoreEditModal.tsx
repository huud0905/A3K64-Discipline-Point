...
function isSheetTotal(event: ScoreEvent) {
  return String(event.note || "").includes("__SHEET_TOTAL__");
}
function parseDay(title: string) {
  const text = title.toLowerCase();
  const match = text.match(/thứ\s*([2-7])/i);
  if (match) return Number(match[1]);
  return text.includes("chủ nhật") ? 0 : null;
}
...
function statusTone(status: string) {
  ...
  return s.includes("tốt") ? "good" : s.includes("khá") ? "warning" : s.includes("đạt") && !s.includes("chưa") ? "orange" : "danger";
}
...
function makeDraft(event: Omit<ScoreEvent, "id">): ScoreEvent {
  return { ...event, id: `draft-${Date.now()}-${Math.random().toString(36).slice(2)}` };
}
function ruleKey(rule: QuickRule) {
  return `${rule.title}::${rule.points}::${rule.category}`;
}
function readPinnedRuleKeys() {
  try {
    const saved = JSON.parse(localStorage.getItem(PINNED_RULES_KEY) || "[]");
    return Array.isArray(saved) ? saved.filter((item) => typeof item === "string") : [];
  } catch {
    return [];
  }
}
function ruleTooltip(rule: QuickRule) {
  return `${rule.title} ${formatScore(rule.points)} · ${categoryLabel(rule.category)}${rule.note ? ` · ${rule.note}` : ""}`;
}
...
export function ScoreEditModal({ student, allStudents = [], week, events, onSaveChanges, onAddScore, onDeleteScore, onClose }: ScoreEditModalProps) {
  ...
  const groupMembers = useMemo(() => {
    const source = allStudents.length ? allStudents : [student];
    return source.filter((item) => item.group === student.group);
  }, [allStudents, student]);
  ...
  const resolveTargetIds = () => {
    ...
    if (bulkScope === "selected") return selectedStudentIds.length ? selectedStudentIds : [student.id];
    return [student.id];
  };

  const targetText = () => {
    const count = resolveTargetIds().length;
    if (bulkScope === "group") return `Cả tổ ${student.group} · ${count} HS`;
    if (bulkScope === "selected") return `Đã chọn ${count} HS`;
    return "Chỉ học sinh này";
  };
  ...
  const stageScore = (payload: { title: string; points: number; category: ScoreCategory }, day = activeDay) => {
    ...
    if (!cleanTitle || !points || isSaving) return;
    ...
    setDraftEvents((current) => [
      ...targetIds.map((studentId) =>
        makeDraft({
          studentId,
          week,
          title: formatSavedTitle(day, payload.category, subject, cleanTitle, points),
          points,
  ...
  const handleSelectedRuleAdd = () => {
    ...
    stageScore({ title: selectedRule.title, points: selectedRule.points, category: selectedRule.category });
  };
  const handleSpecialAdd = () => {
    const points = Number(specialPoint || 0);
    if (!specialTitle.trim() || !Number.isFinite(points) || points === 0) return;
  ...
  const togglePinnedRule = (rule: QuickRule) => {
    const key = ruleKey(rule);
    setPinnedRuleKeys((current) => (current.includes(key) ? current.filter((item) => item !== key) : [key, ...current]));
  };
  ...
  const summaryForDay = (day: number) => {
    const list = visibleWeekEvents.filter((event) => eventDay(event) === day);
    const plus = list.filter((event) => event.points > 0);
    const minus = list.filter((event) => event.points < 0);
    return {
      plus,
      minus,
    ...
    <div className="score-edit-backdrop" role="dialog" aria-modal="true">
      <div className="score-edit-modal modern-score-editor">
        <header className="score-edit-header">
          ...
          <button type="button" className="score-edit-close" onClick={onClose} disabled={isSaving} title="Đóng">
            <X size={20} />
          </button>
        </header>
          ...
          <div className="score-edit-left">
            ...
            <div className="score-edit-columns">
              <div className={`score-add-panel ${section !== "add" ? "mobile-hidden-section" : ""}`}>
                ...
                <div className="score-custom-form special-score-form">
                  <strong>Lỗi / Thưởng đặc biệt khác</strong>
                  <div className="form-row special-row">
                    <input value={specialTitle} onChange={(e) => setSpecialTitle(e.target.value)} placeholder="Nhập ghi chú riêng..." disabled={isSaving} />
                    <input type="number" value={specialPoint} onChange={(e) => setSpecialPoint(Number(e.target.value || 0))} placeholder="Điểm" disabled={isSaving} />
                  </div>
                  <div className="form-row special-row second">
                    <div className="category-picker"><FilterSelect<ScoreCategory> value={category} options={[{ value: "HOC_TAP", label: "Học tập" }, { value: "NE_NEP", label: "Nề nếp" }, { value: "PHONG_TRAO", label: "Phong trào" }]} onChange={setCategory} /></div>
                    <button type="button" className="score-add-button secondary" onClick={handleSpecialAdd} disabled={isSaving}>Thêm lỗi/thưởng khác</button>
                  </div>
                </div>

                <div className="bulk-score-box">
                  <strong>Chấm hàng loạt</strong>
                  <span>Bao gồm học sinh đang chấm</span>
                  <select value={bulkScope} onChange={(event) => setBulkScope(event.target.value as BulkScope)} disabled={isSaving}>
                    <option value="single">Chỉ học sinh này</option>
                    <option value="group">Cả tổ</option>
                    <option value="selected">Chọn học sinh trong tổ</option>
                  </select>
                  {bulkScope === "selected" && <div className="bulk-student-list">{groupMembers.map((item) => <label key={item.id}><input type="checkbox" checked={selectedStudentIds.includes(item.id)} onChange={() => toggleStudent(item.id)} /> <span>{item.name}</span></label>)}</div>}
                  <textarea value={bulkNote} onChange={(event) => setBulkNote(event.target.value)} placeholder="Nhập ghi chú riêng..." />
                  <small>Đối tượng: {targetText()}</small>
                </div>
              </div>
