...
function normalizeRules(raw: unknown): QuickRule[] {
  ...
  return raw.map((item) => {
    ...
    return title && Number.isFinite(rawPoint) ? { title, points: type === "TRU" ? -Math.abs(rawPoint) : Math.abs(rawPoint), type, category: normalizeCategory(record.category ?? record["Phân loại"] ?? record.phanloai), note: String(record.note ?? record["Ghi chú"] ?? record.ghichu ?? "").trim() || undefined } : null;
