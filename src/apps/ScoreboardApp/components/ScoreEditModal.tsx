...
const DRAG_RULE_MIME = "application/x-a3k64-rule";
...
function ruleFromDrag(event: DragEvent) {
  const raw = event.dataTransfer.getData(DRAG_RULE_MIME) || event.dataTransfer.getData("text/plain");
  if (!raw) return null;
  try { return JSON.parse(raw) as QuickRule; } catch { return null; }
}
...
export function ScoreEditModal({ student, allStudents = [], week, events, onSaveChanges, onAddScore, onDeleteScore, onClose }: ScoreEditModalProps) {
  ...
  const activeDayName = days.find((day) => day.key === activeDay)?.full || "Thứ 2";
  ...
  const chooseRule = (rule: QuickRule) => { setSelectedRuleKey(ruleKey(rule)); setRuleSearch(rule.title); setRuleDropdownOpen(false); setCategory(rule.category); setSpecialTitle(rule.title); setSpecialPoint(String(rule.points)); };
  ...
  const handleSpecialAdd = () => { const points = Number(specialPoint.trim()); if (!specialTitle.trim() || !Number.isFinite(points) || points === 0) return; stageScore({ title: specialTitle, points, category }); };
  ...
  const handleRuleDragStart = (event: DragEvent<HTMLButtonElement>, rule: QuickRule) => { event.dataTransfer.effectAllowed = "copy"; const payload = JSON.stringify(rule); event.dataTransfer.setData(DRAG_RULE_MIME, payload); event.dataTransfer.setData("text/plain", payload); };
  const handleRuleDropToSearch = (event: DragEvent) => { const rule = ruleFromDrag(event); if (!rule) return; event.preventDefault(); chooseRule(rule); };
  const handleRuleDropToDay = (event: DragEvent, day: number) => { const rule = ruleFromDrag(event); if (!rule) return; event.preventDefault(); setActiveDay(day); stageScore({ title: rule.title, points: rule.points, category: rule.category }, day); };
  const dayCellProps = (day: number) => ({ role: "button", tabIndex: 0, onClick: () => jumpToDay(day), onDragOver: (event: DragEvent) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }, onDrop: (event: DragEvent) => handleRuleDropToDay(event, day), onKeyDown: (event: KeyboardEvent<HTMLDivElement>) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); jumpToDay(day); } } });

  return (
    <div className="score-edit-backdrop" role="dialog" aria-modal="true">
      <div className="score-edit-modal modern-score-editor">
        <header className="score-edit-header"><div><h2>Chấm điểm: <b>{student.name}</b></h2></div><button type="button" className="score-edit-close" onClick={onClose} disabled={isSaving} title="Đóng"><X size={20} /></button></header>
        <section className="score-edit-body">
          <div className="score-edit-left">
            <div className="score-week-table"><div className="score-edit-section-title">Bảng tổng quan tuần</div><div className="week-matrix"><div className="matrix-cell matrix-head">Nội dung</div>{days.map((day) => <div className="matrix-cell matrix-head day-head matrix-day-jump" key={day.key} {...dayCellProps(day.key)}>{day.label}</div>)}<div className="matrix-cell matrix-label">Điểm (+)</div>{days.map((day) => { const data = summaryForDay(day.key); return <div className="matrix-cell matrix-day-jump" key={`plus-${day.key}`} {...dayCellProps(day.key)}>{data.plusTotal ? formatScore(data.plusTotal) : "-"}</div>; })}<div className="matrix-cell matrix-label">Điểm (-)</div>{days.map((day) => { const data = summaryForDay(day.key); return <div className="matrix-cell matrix-day-jump" key={`minus-${day.key}`} {...dayCellProps(day.key)}>{data.minusTotal ? data.minusTotal : "-"}</div>; })}<div className="matrix-cell matrix-label">Nội dung (+)</div>{days.map((day) => { const data = summaryForDay(day.key); const title = summarizeTitles(data.plus); return <div className="matrix-cell matrix-content matrix-day-jump" key={`plus-title-${day.key}`} title={title} {...dayCellProps(day.key)}><span>{title}</span></div>; })}<div className="matrix-cell matrix-label">Nội dung (-)</div>{days.map((day) => { const data = summaryForDay(day.key); const title = summarizeTitles(data.minus); return <div className="matrix-cell matrix-content matrix-day-jump" key={`minus-title-${day.key}`} title={title} {...dayCellProps(day.key)}><span>{title}</span></div>; })}</div></div>
            <div className="score-day-switch-row"><div className="day-tabs">{days.map((day) => <button type="button" key={day.key} className={activeDay === day.key ? "active" : ""} onClick={() => jumpToDay(day.key)} disabled={isSaving} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }} onDrop={(event) => handleRuleDropToDay(event, day.key)}>{day.label}</button>)}</div><div className="day-record-head inline-day-head"><strong>Ngày {activeDayName}</strong><span>Chỉ ghi vào Google Sheets khi bấm Save all changes.</span></div></div>
            ...
            <div className="score-edit-columns"><div className={`score-add-panel ${section !== "add" ? "mobile-hidden-section" : ""}`}><div className="score-custom-form rule-select-form"><div className="form-row rule-pick-row"><div className="category-picker subject-picker"><FilterSelect<string> value={subject} options={subjects.map((item) => ({ value: item, label: item }))} onChange={setSubject} placement="top" portal menuClassName="subject-floating-menu" menuMaxHeight="none" /></div><div className="rule-search-box" ref={ruleSearchRef} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }} onDrop={handleRuleDropToSearch}><input className="rule-search-input" value={ruleSearch} onChange={(event) => handleRuleSearchChange(event.target.value)} onFocus={() => setRuleDropdownOpen(true)} onKeyDown={handleRuleSearchKeyDown} placeholder="Tìm nội quy" disabled={isSaving} /><span className="rule-search-arrow">▾</span>{ruleDropdownOpen && !isSaving && <div className="rule-suggestion-menu">{filteredRuleSuggestions.length ? filteredRuleSuggestions.map((rule, index) => <button type="button" key={`${ruleKey(rule)}-${index}`} className={rule.points >= 0 ? "plus" : "minus"} onClick={() => chooseRule(rule)}><strong>{rule.title}</strong><span>{rule.points >= 0 ? "Cộng" : "Trừ"}: {Math.abs(rule.points)}đ</span></button>) : <div className="rule-suggestion-empty">Không tìm thấy nội quy phù hợp.</div>}</div>}</div><div className="count-box"><span>Lần</span><input type="number" min={1} step={1} value={violationCount} onChange={(e) => setViolationCount(Math.max(1, Math.trunc(Number(e.target.value) || 1)))} disabled={isSaving} /></div></div><button type="button" className="score-add-button" onClick={handleSelectedRuleAdd} disabled={isSaving || !selectedRule}>Thêm mới (Enter)</button></div><div className="score-custom-form special-score-form"><strong>Lỗi / Thưởng đặc biệt khác</strong><div className="form-row special-row"><input value={specialTitle} onChange={(e) => setSpecialTitle(e.target.value)} placeholder="Nhập lỗi khác..." disabled={isSaving} /><input type="text" inputMode="numeric" value={specialPoint} onChange={(e) => setSpecialPoint(e.target.value)} placeholder="Điểm" disabled={isSaving} /></div><div className="form-row special-row second"><div className="category-picker special-category-picker"><FilterSelect<ScoreCategory> value={category} options={[{ value: "HOC_TAP", label: "Học tập" }, { value: "NE_NEP", label: "Nề nếp" }, { value: "PHONG_TRAO", label: "Phong trào" }]} onChange={setCategory} placement="top" portal menuClassName="category-floating-menu" menuMaxHeight="none" /></div><button type="button" className="score-add-button secondary" onClick={handleSpecialAdd} disabled={isSaving}>Thêm lỗi/thưởng khác</button></div></div><div className="bulk-score-box"><strong>Chấm hàng loạt</strong><span>Bao gồm học sinh đang chấm</span><FilterSelect<BulkScope> value={bulkScope} options={[{ value: "single", label: "Chỉ học sinh này" }, { value: "group", label: "Cả tổ" }, { value: "selected", label: "Chọn học sinh trong tổ" }]} onChange={setBulkScope} placement="top" portal menuClassName="bulk-scope-floating-menu" menuMaxHeight="none" />{bulkScope === "selected" && <div className="bulk-student-list">{groupMembers.map((item) => <label key={item.id}><input type="checkbox" checked={selectedStudentIds.includes(item.id)} onChange={() => toggleStudent(item.id)} /> <span>{item.name}</span></label>)}</div>}<textarea value={bulkNote} onChange={(event) => setBulkNote(event.target.value)} placeholder="Nhập ghi chú riêng..." /><small>Đối tượng: {targetText()}</small></div></div><div className={`day-record-panel ${section !== "review" ? "mobile-hidden-section" : ""}`}><div className="day-event-list" onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }} onDrop={(event) => handleRuleDropToDay(event, activeDay)}>{activeDayEvents.length === 0 ? <div className="empty-day-record">Chưa có nội dung cho ngày này.</div> : activeDayEvents.map((event) => <div key={event.id} className={`day-event ${event.points >= 0 ? "plus" : "minus"} ${event.id.startsWith("draft-") ? "draft" : ""}`}><span>{cleanTitleFromEvent(event.title)}</span><strong>{formatScore(event.points)}</strong><button type="button" onClick={() => removeEvent(event.id)} disabled={isSaving} title="Xoá dòng này"><X size={14} /></button></div>)}</div></div></div>
          </div>
          <aside className="rules-directory"><h3>VI_PHAM</h3>{rulesStatus && <p className="rules-status">{rulesStatus}</p>}<div className="rules-grid">{orderedRules.map((rule, index) => { const pinned = pinnedRuleKeys.includes(ruleKey(rule)); return <button key={`${rule.title}-${index}`} type="button" draggable className={`${rule.points >= 0 ? "rule-card plus" : "rule-card minus"} ${pinned ? "pinned" : ""}`} onClick={() => handleQuickRule(rule)} onDragStart={(event) => handleRuleDragStart(event, rule)} onContextMenu={(e) => { e.preventDefault(); togglePinnedRule(rule); }} data-tooltip={ruleTooltip(rule)} title="Kéo thả vào bảng/tìm nội quy hoặc chuột phải để ghim" disabled={isSaving}><span>{pinned ? "📌 " : ""}{rule.title}</span><strong>{formatScore(rule.points)}</strong></button>; })}</div></aside>
        </section>
