...
type WeekRow = { week: number; total: number; positive: number; negative: number; hocTap: number; neNep: number; phongTrao: number; rank: number; status: string; groupAverage: number; classAverage: number };
...
function alphabetStudents(students: Student[]) { return [...students].sort(compareByGivenName); }
function studentTitle(students: Student[], id: string) { const sorted = alphabetStudents(students); const student = students.find((item) => item.id === id) || sorted.find((item) => item.id === id); const index = Math.max(0, sorted.findIndex((item) => item.id === id)) + 1; return `${pad(index)} - ${student?.name || 'Học sinh'}`; }
function findStudentByIdOrName(students: Student[], id?: string, name?: string) { return students.find((student) => student.id === id) || students.find((student) => normalize(student.name) === normalize(name)); }
...
function StudentProfile({ data, studentId, week, onWeekChange }: { data: DataState; studentId: string; week: number; onWeekChange: (week: number) => void }) {
  ...
  const tableWeekSet = useMemo(() => new Set(tableWeeks), [tableWeeks.join('|')]);
  const selectedHistoryWeeks = historyWeeks.filter((item) => tableWeekSet.has(item)).sort((a, b) => a - b);
  const defaultHistoryWeeks = tableWeekSet.has(activeWeek) ? [activeWeek] : tableWeeks.slice(0, 1);
  const historyWeeksToShow = selectedHistoryWeeks.length ? selectedHistoryWeeks : defaultHistoryWeeks;
  ...
  useEffect(() => { setCompareWeeks((current) => current.filter((item) => availableWeeks.includes(item))); }, [availableWeeks.join('|')]);
  useEffect(() => { setHistoryWeeks((current) => current.filter((item) => tableWeekSet.has(item))); }, [tableWeeks.join('|')]);
  ...
  const weeklyRows: WeekRow[] = buildWeekRows(tableWeeks);
  ...
  const history = allEvents.filter((event) => historyWeeksToShow.includes(event.week)).filter((event) => filter === 'all' || (filter === 'plus' ? event.points > 0 : filter === 'minus' ? event.points < 0 : event.category === filter)).sort((a, b) => b.week - a.week || Date.parse(b.createdAt || '') - Date.parse(a.createdAt || ''));
  ...
  const toggleHistoryWeek = (targetWeek: number) => setHistoryWeeks((current) => {
    const currentSelected = current.filter((item) => tableWeekSet.has(item));
    const base = currentSelected.length ? currentSelected : defaultHistoryWeeks;
    const next = base.includes(targetWeek) ? base.filter((item) => item !== targetWeek) : [...base, targetWeek];
    return next.filter((item) => tableWeekSet.has(item)).sort((a, b) => a - b);
  });

  return <div className="profile-page"><section className="profile-hero"><div className="profile-avatar-big">{student.avatarInitial || student.name[0]}</div><div><span>Hồ sơ học sinh · Tuần {activeWeek}</span><h1>{student.name}</h1><p>Tổ {student.group} · {student.role || 'Học sinh'} · {student.status}</p></div><strong>#{student.rank}<small>Hạng lớp</small></strong></section><section className="profile-stat-grid">{[['Tổng điểm', formatScore(student.total)], ['Điểm cộng', formatScore(student.positive)], ['Điểm trừ', String(student.negative)], ['Hạng tổ', `#${groupRank}/${groupMembers.length}`], ['Học tập', formatScore(categoryTotal(weekEvents, 'HOC_TAP'))], ['Nề nếp', formatScore(categoryTotal(weekEvents, 'NE_NEP'))], ['Phong trào', formatScore(categoryTotal(weekEvents, 'PHONG_TRAO'))], ['TB tổ', String(groupAverage)]].map(([label, value]) => <article key={label}><span>{label}</span><b className={String(value).startsWith('-') ? 'negative' : 'positive'}>{value}</b></article>)}</section><section className="profile-grid"><article className="profile-card wide"><h2><BarChart3 size={18} /> Biểu đồ so sánh tuần</h2><ProfileChart rows={chartRows} /></article><article className="profile-card"><h2>Nhận xét tự động</h2><ul>{notes.map((note) => <li key={note}>{note}</li>)}</ul><h2>So sánh trong tổ</h2><p>Cách người trên: {above ? `${Math.max(0, above.total - student.total)} điểm` : 'Đang dẫn đầu'}</p><p>Cách người dưới: {below ? `${Math.max(0, student.total - below.total)} điểm` : 'Cuối nhóm'}</p><p>So với TB tổ: {formatScore(student.total - groupAverage)}</p></article></section><section className="profile-card profile-week-overview-card"><div className="profile-history-head profile-compare-head"><h2>Tổng quan tuần so sánh</h2><div className="profile-week-tabs"><div className="profile-week-picker-anchor"><button type="button" onClick={() => setPickerOpen((open) => !open)} className={pickerOpen ? 'active' : ''}>Chọn tuần so sánh</button>{pickerOpen && <div className="profile-week-picker"><div>{availableWeeks.map((itemWeek) => <button key={itemWeek} type="button" className={tableWeeks.includes(itemWeek) ? 'selected' : ''} onClick={() => toggleWeek(itemWeek)}>Tuần {itemWeek}</button>)}</div></div>}</div></div></div><div className="profile-table-wrap"><table className="profile-week-table"><thead><tr><th>Lịch sử</th><th>Tuần</th><th>Tổng</th><th>Cộng</th><th>Trừ</th><th>Học tập</th><th>Nề nếp</th><th>Phong trào</th><th>Hạng lớp</th><th>Xếp loại</th></tr></thead><tbody>{weeklyRows.map((row) => <tr key={row.week} className={row.week === activeWeek ? 'active-week-row' : ''} onClick={() => onWeekChange(row.week)}><td><label className="profile-week-check" title="Chọn tuần này để xem lịch sử bên dưới" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={historyWeeksToShow.includes(row.week)} onChange={() => toggleHistoryWeek(row.week)} /><span /></label></td><td>T{row.week}</td><td className={row.total >= 0 ? 'positive' : 'negative'}>{formatScore(row.total)}</td><td className="positive">{formatScore(row.positive)}</td><td className={row.negative < 0 ? 'negative' : ''}>{row.negative}</td><td>{formatScore(row.hocTap)}</td><td>{formatScore(row.neNep)}</td><td>{formatScore(row.phongTrao)}</td><td>{row.rank ? `#${row.rank}` : '-'}</td><td><div className="profile-status-radio-cell"><span>{row.status}</span><label className="profile-week-radio" title="Chọn tuần này để xem hồ sơ phía trên"><input type="radio" name={`profile-week-${student.id}`} checked={row.week === activeWeek} onChange={() => onWeekChange(row.week)} /><span /></label></div></td></tr>)}</tbody></table></div></section><section className="profile-card"><div className="profile-history-head"><h2>{historyTitle}</h2><div>{[['all', 'Tất cả'], ['plus', 'Cộng'], ['minus', 'Trừ'], ['HOC_TAP', 'Học tập'], ['NE_NEP', 'Nề nếp'], ['PHONG_TRAO', 'Phong trào']].map(([value, label]) => <button key={value} className={filter === value ? 'active' : ''} onClick={() => setFilter(value)}>{label}</button>)}</div></div><div className="profile-table-wrap"><table><thead><tr><th>Tuần</th><th>Nội dung</th><th>Điểm</th><th>Loại</th><th>Người nhập</th><th>Thời gian</th></tr></thead><tbody>{history.length ? history.map((event) => <tr key={event.id}><td>T{event.week}</td><td>{event.title}</td><td className={event.points >= 0 ? 'positive' : 'negative'}>{formatScore(event.points)}</td><td>{categoryLabel(event.category)}</td><td>{event.createdBy || 'Google Sheets'}</td><td>{event.createdAt ? new Date(event.createdAt).toLocaleString('vi-VN') : 'Chưa rõ'}</td></tr>) : <tr><td colSpan={6}>{historyTitle} chưa có dữ liệu phù hợp.</td></tr>}</tbody></table></div></section></div>;
}
