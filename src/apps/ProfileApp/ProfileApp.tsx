  ...
  try {
    ...
    return (
      students.find((student) =>
        names.some((name) => normalize(student.name) === name || normalize(student.name).includes(name) || name.includes(normalize(student.name))),
      ) || students[0]
    );
  } catch {
    return students[0];
...
async function loadProfileData(): Promise<DataState> {
  ...
  if (remote?.students?.length) {
    return {
      ...
      weeks: remote.weeks.length ? remote.weeks : SCORE_WEEKS,
      source: 'gas',
    };
  }
  ...
  try {
    ...
    return {
      students: mockStudents,
      events: events?.length ? events : mockScoreEvents,
      weeks: weeks?.length ? weeks : SCORE_WEEKS,
      source: 'local',
    };
  } catch {
    return DEFAULT_DATA;
...
function StudentProfile({ data, studentId, week, onWeekChange }: { data: DataState; studentId: string; week: number; onWeekChange: (week: number) => void }) {
  ...
  useEffect(() => {
    setHistoryWeeks((current) => {
      ...
      if (tableWeekSet.has(activeWeek)) return [activeWeek];
      return tableWeeks.slice(0, 1);
    });
  }, [tableWeekKey, activeWeek]);
  ...
  const historyWeeksToShow = useMemo(() => {
    ...
    if (tableWeekSet.has(activeWeek)) return [activeWeek];
    return tableWeeks.slice(0, 1);
  }, [historyWeeks, tableWeekKey, activeWeek]);
  ...
  const buildWeekRows = (sourceWeeks: number[]): WeekRow[] =>
    sourceWeeks.map((itemWeek) => {
  ...
  const historyTitle = historyWeeksToShow.length > 1
    ? `Lịch sử điểm các tuần ${historyWeeksToShow.map((item) => `T${item}`).join(', ')}`
    : `Lịch sử điểm tuần ${historyWeeksToShow[0] || activeWeek}`;
  const notes = [
  ...
  const toggleHistoryWeek = (targetWeek: number) => {
    setHistoryWeeks((current) => {
      const cleaned = current.filter((item) => tableWeekSet.has(item));
      const base = cleaned.length ? cleaned : historyWeeksToShow;
      const next = base.includes(targetWeek) ? base.filter((item) => item !== targetWeek) : [...base, targetWeek];
  ...
  return (
    <div className="profile-page">
      ...
      <section className="profile-stat-grid">
        {[
          ['Tổng điểm', formatScore(student.total)],
          ['Điểm cộng', formatScore(student.positive)],
          ['Điểm trừ', String(student.negative)],
          ['Hạng tổ', `#${groupRank}/${groupMembers.length}`],
          ['Học tập', formatScore(categoryTotal(weekEvents, 'HOC_TAP'))],
          ['Nề nếp', formatScore(categoryTotal(weekEvents, 'NE_NEP'))],
          ['Phong trào', formatScore(categoryTotal(weekEvents, 'PHONG_TRAO'))],
          ['TB tổ', String(groupAverage)],
        ].map(([label, value]) => (
          <article key={label}>
            <span>{label}</span>
            <b className={String(value).startsWith('-') ? 'negative' : 'positive'}>{value}</b>
          </article>
        ))}
      </section>
      ...
      <section className="profile-grid">
        <article className="profile-card wide">
          <h2><BarChart3 size={18} /> Biểu đồ so sánh tuần</h2>
          <ProfileChart rows={chartRows} />
        </article>
        <article className="profile-card">
          <h2>Nhận xét tự động</h2>
          <ul>{notes.map((note) => <li key={note}>{note}</li>)}</ul>
          <h2>So sánh trong tổ</h2>
          <p>Cách người trên: {above ? `${Math.max(0, above.total - student.total)} điểm` : 'Đang dẫn đầu'}</p>
          <p>Cách người dưới: {below ? `${Math.max(0, student.total - below.total)} điểm` : 'Cuối nhóm'}</p>
          <p>So với TB tổ: {formatScore(student.total - groupAverage)}</p>
        </article>
      </section>

      <section className="profile-card profile-week-overview-card">
        <div className="profile-history-head profile-compare-head">
          <h2>Tổng quan tuần so sánh</h2>
          <div className="profile-week-tabs">
            <div className="profile-week-picker-anchor">
              <button type="button" onClick={() => setPickerOpen((open) => !open)} className={pickerOpen ? 'active' : ''}>Chọn tuần so sánh</button>
              {pickerOpen && (
                <div className="profile-week-picker">
                  <div>
                    {availableWeeks.map((itemWeek) => (
                      <button key={itemWeek} type="button" className={tableWeeks.includes(itemWeek) ? 'selected' : ''} onClick={() => toggleWeek(itemWeek)}>
                        Tuần {itemWeek}
                      </button>
                    ))}
                  </div>
        ...
        <div className="profile-table-wrap">
          <table className="profile-week-table">
            <thead>
              <tr>
                <th>Lịch sử</th>
                <th>Tuần</th>
            ...
            <tbody>
              {weeklyRows.map((row) => {
                const checked = historyWeeksToShow.includes(row.week);
                return (
                  <tr key={row.week} className={row.week === activeWeek ? 'active-week-row' : ''} onClick={() => onWeekChange(row.week)}>
                    <td>
                      <button
                        type="button"
                        className={`profile-week-toggle ${checked ? 'selected' : ''}`}
                        aria-pressed={checked}
                        title="Chọn tuần này để xem lịch sử bên dưới"
                        onClick={(event) => {
                          event.preventDefault();
                          event.stopPropagation();
                          toggleHistoryWeek(row.week);
                        }}
                      >
                        <span>{checked ? '✓' : ''}</span>
                      </button>
                    ...
                    <td>T{row.week}</td>
                    <td className={row.total >= 0 ? 'positive' : 'negative'}>{formatScore(row.total)}</td>
                    <td className="positive">{formatScore(row.positive)}</td>
                    <td className={row.negative < 0 ? 'negative' : ''}>{row.negative}</td>
                    <td>{formatScore(row.hocTap)}</td>
                    <td>{formatScore(row.neNep)}</td>
                    <td>{formatScore(row.phongTrao)}</td>
            ...
            <tbody>
              {history.length ? history.map((event) => (
                <tr key={event.id}>
                  <td>T{event.week}</td>
                  <td>{event.title}</td>
                  <td className={event.points >= 0 ? 'positive' : 'negative'}>{formatScore(event.points)}</td>
                  <td>{categoryLabel(event.category)}</td>
                  <td>{event.createdBy || 'Google Sheets'}</td>
                  <td>{event.createdAt ? new Date(event.createdAt).toLocaleString('vi-VN') : 'Chưa rõ'}</td>
                </tr>
              )) : (
                <tr><td colSpan={6}>{historyTitle} chưa có dữ liệu phù hợp.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function NewProfileTab({ query, setQuery, results, onOpenStudent, onRefresh }: { query: string; setQuery: (value: string) => void; results: Student[]; onOpenStudent: (id: string) => void; onRefresh: () => void }) {
  return (
    <div className="profile-new-page">
      <div className="profile-browser-bar">
        <button type="button" className="profile-browser-nav" title="Quay lại" disabled><ArrowLeft size={17} /></button>
        <button type="button" className="profile-browser-nav" title="Tiến lên" disabled><ArrowRight size={17} /></button>
        <button type="button" className="profile-browser-nav" title="Làm mới dữ liệu" onClick={onRefresh}><RotateCw size={16} /></button>
        <label className="profile-address-bar">
          <Search size={18} />
          <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm học sinh..." />
        </label>
      </div>
      <div className="profile-new-content">
        {query && (
          <div className="profile-new-results">
            {results.length ? results.map((student) => (
              <button key={student.id} type="button" onClick={() => { onOpenStudent(student.id); setQuery(''); }}>
                <span>{student.name}</span>
                <small>Tổ {student.group}</small>
              </button>
            )) : <div className="profile-new-empty-result">Không tìm thấy học sinh phù hợp.</div>}
          </div>
        )}
      </div>
    </div>
...
export default function ProfileApp({ userName, userEmail, requestedStudentId, requestedWeek }: ProfileAppProps) {
  ...
  const openStudent = (id?: string, targetWeek?: number) => {
    ...
    setTabs((current) => {
      ...
      const nextStudentTab: ProfileTab = {
        key: activeTab?.kind === 'new' ? activeTab.key : `student-${student.id}-${Date.now()}`,
        kind: 'student',
        id: student.id,
        name: student.name,
        title: studentTitle(data.students, student.id),
      };
  ...
  const pinTab = (key: string) => {
    setTabs((current) => {
      ...
      setActiveKey(key);
      return [target, ...current.filter((tab) => tab.key !== key)];
    });
  };
  ...
  const closeTabsBelow = (key: string) => {
    setTabs((current) => {
      const index = current.findIndex((tab) => tab.key === key);
  ...
  const toggleVerticalTabs = () => {
    setVerticalTabs((current) => {
      const next = !current;
      localStorage.setItem(VERTICAL_TABS_KEY, next ? 'on' : 'off');
      return next;
    });
  };
  ...
  useEffect(() => {
    if (data.source !== 'loading' && data.students.length && !tabs.length) openStudent(requestedStudentId);
  }, [data.source, data.students.length]);

  useEffect(() => {
    if (data.source !== 'loading' && requestedStudentId) openStudent(requestedStudentId, requestedWeek);
  }, [requestedStudentId, requestedWeek, data.source]);

  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 't') {
        event.preventDefault();
        openNewTab(activeKey);
      }
  ...
  const results = alphabetStudents(data.students)
    .filter((student) => normalize(student.name).includes(normalize(query)) || String(student.group).includes(query.trim()))
    .slice(0, 10);
  ...
  return (
    <div className={`profile-app-shell ${verticalTabs ? 'vertical-tabs' : 'horizontal-tabs'}`} onContextMenu={(event) => event.preventDefault()}>
      <aside className="profile-sidebar">
        <div className="profile-sidebar-top">
          <p className="profile-side-title">Tab học sinh</p>
          <button type="button" className="profile-mini-new-tab" onClick={() => openNewTab(activeKey)} title="Mở tab mới"><Plus size={15} /></button>
        </div>
        <div className="profile-tabs">
          {tabs.map((tab) => (
            <button
              key={tab.key}
              type="button"
              className={`${tab.key === activeTab?.key ? 'active' : ''} ${tab.kind === 'new' ? 'new-tab' : ''}`}
            ...
            >
              <span>{tab.title}</span>
              <X size={13} onClick={(event) => { event.stopPropagation(); closeTab(tab.key); }} />
            </button>
          ))}
        </div>
        <button type="button" className="profile-new-tab-button" onClick={() => openNewTab(activeKey)}>
          <Plus size={16} />
          <span>Tab mới</span>
          <kbd>Ctrl+T</kbd>
        </button>
      ...
      <main className="profile-main">
        {activeTab?.kind === 'new' ? (
          <NewProfileTab query={query} setQuery={setQuery} results={results} onOpenStudent={openStudent} onRefresh={() => void refresh()} />
        ) : activeStudent ? (
          <StudentProfile data={data} studentId={activeStudent} week={week} onWeekChange={setWeek} />
        ) : (
          <div className="profile-empty">Bấm “Tab mới” để tìm và mở hồ sơ học sinh.</div>
        )}
      </main>
      {tabMenu && menuTab && (
        <div className="profile-tab-context-menu" style={{ left: tabMenu.x, top: tabMenu.y }} onClick={(event) => event.stopPropagation()} onContextMenu={(event) => event.preventDefault()}>
