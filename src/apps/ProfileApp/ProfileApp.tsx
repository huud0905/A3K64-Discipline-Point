...
function StudentProfile({ data, studentId, week, onWeekChange }: { data: DataState; studentId: string; week: number; onWeekChange: (week: number) => void }) {
  ...
  const historyWeeksToShow = selectedHistoryWeeks.length ? selectedHistoryWeeks : (tableWeekSet.has(activeWeek) ? [activeWeek] : tableWeeks.slice(0, 1));
