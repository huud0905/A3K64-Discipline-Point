...
function cloneSeating(value: SeatingState): SeatingState {
  return {
    left: value.left.map((row) => row.slice()),
    right: value.right.map((row) => row.slice()),
  };
}
...
function loadSeating(): SeatingState {
  try {
    ...
    return {
      left: parsed.left.map((row) => Array.isArray(row) ? row.slice(0, 4) : ["", "", "", ""]),
      right: parsed.right.map((row) => Array.isArray(row) ? row.slice(0, 4) : ["", "", "", ""]),
    };
  } catch {
...
function injectSeatingStyles() {
  ...
  style.textContent = `
    .a3-seat-window{
      position:absolute;
      left:calc(50% - min(600px,calc((100vw - 176px)/2)));
      top:24px;
      width:min(1200px,calc(100vw - 176px));
      height:min(720px,calc(100vh - 104px));
      min-height:560px;
      border:1px solid color-mix(in srgb,var(--desktop-accent,#2563eb) 38%,#273244);
      border-radius:22px;
      overflow:hidden;
      background:#07111f;
      color:#f8fafc;
      ...
      z-index:90;
      animation:seatWindowIn .18s ease both;
    }
    @keyframes seatWindowIn{from{opacity:0;transform:translateY(18px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}
    .a3-seat-window.maximized{position:fixed;left:0!important;top:0!important;width:100vw!important;height:calc(100vh - 58px)!important;border-radius:0;}
    ...
    .a3-seat-body{height:calc(100% - 46px);overflow:auto;padding:18px;background:radial-gradient(circle at 12% 12%,color-mix(in srgb,var(--desktop-accent,#2563eb) 18%,transparent),transparent 32%),#050914;}
    ...
    .a3-seat-heading h1{margin:0;font-size:23px;letter-spacing:-.04em;}
    ...
    .a3-seat-tools input{height:38px;width:190px;border:1px solid #273244;border-radius:13px;background:#0b1220;color:#f8fafc;padding:0 12px;outline:none;}
    ...
    .a3-seat-board{border:1px solid #273244;border-radius:24px;padding:18px;background:rgba(15,23,42,.82);box-shadow:inset 0 1px 0 rgba(255,255,255,.04);}
    .a3-seat-back,.a3-seat-front{display:flex;align-items:center;justify-content:center;gap:18px;font-weight:1000;color:#e0f2fe;letter-spacing:.05em;}
    ...
    .a3-seat-cell{min-height:42px;border:0;border-right:1px solid #334155;background:#0b1220;color:#f8fafc;display:flex;align-items:center;justify-content:center;padding:7px 5px;text-align:center;font-weight:800;font-size:14px;line-height:1.2;cursor:default;position:relative;}
    .a3-seat-cell:last-child{border-right:0;}
    .a3-seat-cell.empty{opacity:.36;background:rgba(15,23,42,.62);}
    ...
    .a3-seat-cell.drag-over{background:rgba(34,197,94,.2);box-shadow:inset 0 0 0 2px #22c55e;}
    ...
    .theme-light .a3-seat-title-left span{color:#64748b;}
    .theme-light .a3-seat-body{background:linear-gradient(180deg,#f8fafc,#edf2f7);}
    .theme-light .a3-seat-board{background:#fff;border-color:#dbe4f0;}
    .theme-light .a3-seat-row{background:#fff;border-color:#cbd5e1;}
    .theme-light .a3-seat-cell{background:#fff;color:#0f172a;border-right-color:#cbd5e1;}
    .theme-light .a3-seat-cell.empty{background:#f1f5f9;}
    .theme-light .a3-seat-tools input,.theme-light .a3-seat-tools button{background:#fff;color:#0f172a;border-color:#cbd5e1;}
    .theme-light .a3-seat-aisle{background:#f8fafc;border-color:#cbd5e1;color:#2563eb;}
    @media (max-width:760px){
      .a3-seat-window{position:fixed;left:0!important;top:0!important;width:100vw!important;height:calc(100vh - 58px)!important;min-height:0;border-radius:0;}
      .a3-seat-body{padding:12px;}
      .a3-seat-toolbar{display:grid;gap:10px;}
      .a3-seat-tools{justify-content:flex-start;}
      .a3-seat-tools input{width:100%;}
      .a3-seat-layout{grid-template-columns:1fr;gap:12px;}
      .a3-seat-aisle{writing-mode:horizontal-tb;height:34px;}
      .a3-seat-front{grid-template-columns:1fr;gap:8px;}
      .a3-seat-row{min-width:520px;}
      .a3-seat-side{overflow-x:auto;padding-bottom:4px;}
      .a3-seat-mobile-note{display:block;}
    }
    @media print{
      body *{visibility:hidden!important;}
      .a3-seat-window,.a3-seat-window *{visibility:visible!important;}
      .a3-seat-window{position:fixed!important;left:0!important;top:0!important;width:100vw!important;height:auto!important;box-shadow:none!important;border:0!important;background:#fff!important;color:#000!important;}
      .a3-seat-titlebar,.a3-seat-toolbar .a3-seat-tools{display:none!important;}
      .a3-seat-body{height:auto!important;overflow:visible!important;background:#fff!important;padding:20px!important;}
      .a3-seat-board{box-shadow:none!important;background:#fff!important;border-color:#111!important;}
      .a3-seat-cell{background:#fff!important;color:#000!important;border-color:#111!important;}
      .a3-seat-row{border-color:#111!important;}
    }
  `;
...
function openSeatingMapApp() {
  ...
  win.innerHTML = `
    <header class="a3-seat-titlebar">
      ...
      <div class="a3-seat-actions">
        <button type="button" data-action="maximize" title="Phóng to">□</button>
        <button type="button" class="danger" data-action="close" title="Đóng">×</button>
      </div>
    </header>
  ...
  let editMode = false;
  let query = "";
  let dragged: SeatPosition | null = null;

  const body = win.querySelector(".a3-seat-body") as HTMLElement;

  const render = () => {
    const q = normalizeText(query);
    const total = state.left.flat().concat(state.right.flat()).filter(Boolean).length;
    body.innerHTML = `
      <div class="a3-seat-toolbar">
        <div class="a3-seat-heading">
          <span class="a3-seat-badge">Thay đổi lần 1 · Áp dụng 5/9/2025</span>
          <div><h1>SƠ ĐỒ CHỖ NGỒI LỚP 11A3</h1><p>${total} học sinh · kéo thả khi bật chế độ sửa · dữ liệu lưu tạm trên máy</p></div>
        </div>
        <div class="a3-seat-tools">
          <input data-search="1" value="${escapeHtml(query)}" placeholder="Tìm học sinh..." />
          <button type="button" data-tool="edit" class="${editMode ? "primary" : ""}">${editMode ? "Đang sửa" : "Bật sửa"}</button>
          <button type="button" data-tool="reset">Khôi phục</button>
          <button type="button" data-tool="print">Xuất/in</button>
        </div>
      </div>
      <div class="a3-seat-board">
        <div class="a3-seat-back">CUỐI LỚP</div>
        <div class="a3-seat-layout">
          <div class="a3-seat-side">${renderSide("left", state.left, q, editMode)}</div>
          <div class="a3-seat-aisle">LỐI ĐI</div>
          <div class="a3-seat-side">${renderSide("right", state.right, q, editMode)}</div>
        </div>
        <div class="a3-seat-front"><span class="teacher">BÀN GV</span><span>BẢNG</span><span class="door">CỬA RA VÀO</span></div>
        <div class="a3-seat-approve">GVCN: Võ Thị Ngọc Tân – Đã Duyệt</div>
        <div class="a3-seat-mobile-note">Trên điện thoại có thể kéo ngang từng dãy để xem đủ 4 chỗ/bàn.</div>
      </div>
    `;
    ...
    body.querySelectorAll<HTMLElement>(".a3-seat-cell").forEach((cell) => {
      ...
      cell.addEventListener("drop", (event) => {
        ...
        const sourceName = seatAt(state, dragged);
        const targetName = seatAt(state, target);
        setSeat(state, dragged, targetName);
        setSeat(state, target, sourceName);
        saveSeating(state);
        dragged = null;
        render();
      });
  ...
  win.querySelector<HTMLElement>("[data-action='close']")?.addEventListener("click", () => win.remove());
  win.querySelector<HTMLElement>("[data-action='maximize']")?.addEventListener("click", () => win.classList.toggle("maximized"));
  win.addEventListener("mousedown", () => { win.style.zIndex = String(nextSeatingZ()); });
}

function renderSide(side: "left" | "right", rows: string[][], query: string, editMode: boolean) {
  return rows.map((row, rowIndex) => `
    <div class="a3-seat-row">
      ${row.map((name, seatIndex) => {
...
function ensureDesktopShortcut() {
  ...
  button.addEventListener("click", (event) => {
    if ((event as MouseEvent).detail === 1) window.setTimeout(() => undefined, 160);
  });
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", bootSeatingRuntimeApp);
} else {
  bootSeatingRuntimeApp();
}
