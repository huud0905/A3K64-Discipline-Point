...
và vẫn điền "suggested_title" đã sửa viết tắt trong cùng lúc.
Nếu mục không có vấn đề gì (đã đúng loại/môn, không viết tắt) → issue = "none",
để "suggested_title" giữ nguyên nội dung gốc và "reason" để trống.
...
[
  ...
  async function _runAudit() {
    ...
    try {
      ...
      const newSug = _buildSuggestions(parsedList, result);
      if (newSug.length) {
  ...
  function _buildSuggestions(parsedList, result) {
    const out = [];
    ...
    result.forEach(r => {
      ...
      if (sameCat && sameSubj && sameTitle) return; // AI báo có vấn đề nhưng thực ra không đổi gì → bỏ qua
      ...
      let newTitle;
      try { newTitle = formatSavedTitle(day, suggestedCategory, suggestedSubject || (subjectsList[0] || ''), suggestedContentTitle, ev.points); }
      catch { return; }
      ...
      out.push({
        ...
        // Giữ lại loại/môn GỐC (trước khi sửa) — cần để có thể Hoàn tác đúng
        // như cũ nếu sau này phát hiện AI sửa sai.
  ...
  async function _applyFix(sugId) {
    ...
    try {
      ...
      await saveScoreChanges({
        ...
        deletions: [{ id: s.id, studentId: s.studentId, week: s.week, title: s.oldTitle, points: s.points }],
      });
      ...
      try {
        (state.events || []).forEach(e => {
          ...
          if (!beforeIds.has(e.id) && e.studentId === s.studentId && e.week === s.week
            && e.title === s.newTitle && e.points === s.points) newId = e.id;
        });
    ...
    try {
      ...
      await saveScoreChanges({
        additions: [{
          ...
          points: h.points, type: h.points >= 0 ? 'CONG' : 'TRU',
      ...
      _dismissed.add(_dismissKey(h.studentId, h.week, h.oldTitle, h.points));
      _saveSet(DISMISSED_KEY, _dismissed);
      try {
        const restored = (state.events || []).find(e => !beforeIds.has(e.id)
          && e.studentId === h.studentId && e.week === h.week && e.title === h.oldTitle && e.points === h.points);
  ...
  function _issueLabel(t) { return t === 'abbr' ? 'Viết tắt' : 'Sai loại/môn'; }
    ...
    const rows = _suggestions.map(s => `
      <div class="a3-audit-row" data-id="${_esc(s.id)}">
        <div class="a3-audit-row-top">
          ...
          <span class="a3-audit-tag ${s.issueType === 'abbr' ? 'abbr' : 'cat'}">${_esc(_issueLabel(s.issueType))}</span>
        </div>
