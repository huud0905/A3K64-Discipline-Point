...
(function A3AIParser() {
   ...
   */
  const AI_DEFAULT_KEY  = 'AQ.Ab8RN6I-OyE_I6c_cT2jq4AE6YohbKx13JNUmo7acdiX3q02MA';
  ...
  /** Kiểm tra key hiện tại có phải key mặc định (hardcoded) không */
  ...
  async function _callGemini(text) {
    ...
    if (!key || key === 'PASTE_YOUR_GEMINI_KEY_HERE') {
      throw new Error('Chưa có Gemini API Key. Nhập key ở bên dưới hoặc liên hệ admin để cấu hình.');
    }
...
<div class="ai-backdrop" id="ai-backdrop">
  <div class="ai-modal" role="dialog" aria-modal="true" aria-label="Tự tính điểm bằng AI">
...
Cả tổ 3 vắng chào cờ trừ 100 mỗi người"
  ...
  function _refreshPreview() {
    ...
    if (applyBtn) {
      ...
      const cnt = applyBtn.querySelector('.ai-apply-count');
      if (cnt) cnt.textContent = _results.length;
    }
    root.querySelector('#ai-preview-wrap')?.addEventListener('change', _onPreviewChange);
    root.querySelector('#ai-preview-wrap')?.addEventListener('input', _onPreviewInput);
    root.querySelector('#ai-preview-wrap')?.addEventListener('click', _onPreviewClick);
  ...
  ──────────────────────────────────────────────────────── */
  function _injectCSS() {
    if (document.getElementById('a3-ai-css')) return;
...
.ai-header-icon {
  ...
  background: linear-gradient(135deg,
    color-mix(in srgb, var(--accent) 28%, #0a1428),
    color-mix(in srgb, var(--accent) 10%, #050d1c));
...
.ai-close-btn {
  ...
  background: color-mix(in srgb, var(--text) 400.0%, transparent);
...
.ai-row:hover { background: color-mix(in srgb, var(--text) 4500.0%, transparent); }
...
.ai-day-badge {
  ...
  background: color-mix(in srgb, var(--accent) 14%, #0a1428);
...
.ai-reason-input:hover { border-color: rgba(148,163,184,.18); background: color-mix(in srgb, var(--text) 400.0%, transparent); }
.ai-reason-input:focus { outline: none; border-color: var(--accent); background: color-mix(in srgb, var(--text) 400.0%, transparent); }
...
.ai-row-del {
  ...
  border: 0; background: color-mix(in srgb, var(--text) 300.0%, transparent); color: var(--text-dim);
...
.ai-cancel-btn {
  ...
  border: 1px solid rgba(148,163,184,.14);
  background: color-mix(in srgb, var(--text) 400.0%, transparent); color: var(--text-muted);
  font-size: 14px; font-weight: 700; cursor: pointer;
  transition: all .14s ease;
}
.ai-cancel-btn:hover { background: color-mix(in srgb, var(--text) 800.0%, transparent); color: var(--text-muted); }
.ai-apply-btn {
...
.ai-apply-count {
  ...
  background: color-mix(in srgb, var(--text) 2200.0%, transparent); font-size: 11.5px; font-weight: 900;
...
.ai-success-close {
  ...
  background: color-mix(in srgb, var(--text) 600.0%, transparent); color: var(--text-muted);
...
.ai-key-eye-btn:hover { color: var(--text-muted); background: color-mix(in srgb, var(--text) 700.0%, transparent); }

.ai-key-reset-btn {
  height: 38px; padding: 0 11px; border-radius: 10px;
  border: 1px solid rgba(148,163,184,.14);
  background: color-mix(in srgb, var(--text) 300.0%, transparent); color: var(--text-muted);
...
@media (max-width: 700px) {
  ...
  .ai-left { border-right: 0; border-bottom: 1px solid rgba(148,163,184,.08); }
}
    `;
