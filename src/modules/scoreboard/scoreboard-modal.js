...
(function A3ModalV2() {
  ...
  let _activeDay = 2;        // key của ngày đang xem (2-7, 0=CN)
    ...
    <div class="v2-body" id="v2-body">
      ...
      <div class="v2-right">
        <div class="v2-rules-sidebar">
          <div class="v2-rules-label">Nội quy nhanh <small>Chuột phải để ghim</small></div>
  ...
  function _patchRulesGrid() {
    ...
    el.innerHTML = rules.map(rule => {
      ...
      return `<button type="button" class="v2-rule-card ${rule.points >= 0 ? 'plus' : 'minus'}${pinned ? ' pinned' : ''}"
        data-rule="${ruleJson}" title="${_esc(rule.title)} (${rule.points >= 0 ? 'Cộng' : 'Trừ'} ${Math.abs(rule.points)}đ) — Chuột phải để ghim">
...
.v2-rule-card.minus:hover {
  ...
  box-shadow: 0 8px 22px rgba(239,68,68,.14);
}
.v2-rule-card.pinned {
...
.v2-ev-item:hover { background: color-mix(in srgb, var(--text) 400.0%, transparent); }
.v2-ev-item.plus  { background: rgba(16,185,129,.10); border-left: 3px solid rgba(52,211,153,.45); }
.v2-ev-item.minus { background: rgba(239,68,68,.10);  border-left: 3px solid rgba(248,113,113,.45); }
...
.v2-ev-item.plus  .v2-ev-title { color: #6ee7b7; }
.v2-ev-item.minus .v2-ev-title { color: #fca5a5; }
.v2-ev-pts {
  padding: 9px 12px;
  font-size: 15px; font-weight: 900; white-space: nowrap;
}
.v2-ev-item.plus  .v2-ev-pts { color: #10b981; }
.v2-ev-item.minus .v2-ev-pts { color: #ef4444; }
/* Cột ngày (tab Tất cả) */
.v2-ev-day {
  padding: 9px 4px;
  font-size: 12px; font-weight: 800; white-space: nowrap;
}
.v2-ev-item.plus  .v2-ev-day { color: rgba(110,231,183,.7); }
.v2-ev-item.minus .v2-ev-day { color: rgba(252,165,165,.7); }
/* Tab Tất cả */
...
@media (max-width: 900px) {
  .v2-body { grid-template-columns: 1fr; overflow-y: auto; }
  .v2-col-resizer { display: none; }
  .v2-left { border-right: 0; border-bottom: 1px solid rgba(148,163,184,.08); padding: 14px 16px; }
  .v2-middle { border-right: 0; border-bottom: 1px solid rgba(148,163,184,.08); max-height: 46vh; }
  .v2-right { border-top: 0; border-left: 0; max-height: 46vh; }
  .v2-rules-grid { grid-template-columns: repeat(3, 1fr); }
}
@media (max-width: 480px) {
  .v2-modal { border-radius: 0; }
  .v2-header { padding: 12px 14px; }
  .v2-score-chip, .v2-score-status { height: 28px; padding: 0 8px; font-size: 11.5px; }
  .v2-footer { padding: 10px 14px; }
