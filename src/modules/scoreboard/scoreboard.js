...
const DATA_CACHE_MAX_AGE_MS = 5 * 60 * 1000; // Cache còn dùng được tối đa 5 phút
const LIVE_REFRESH_MS       = 7000;          // Chu kỳ polling dữ liệu mới
...
const GAS_REFRESH_DELAY_MS  = 9500;          // Độ trễ trước khi tải lại dữ liệu sau khi lưu
...
const SCORE_STATUS = { GOOD: 'Tốt', FAIR: 'Khá', PASS: 'Đạt', FAIL: 'Chưa đạt' };
...
// studentId, week, title, points, type. KHÔNG gồm category/note — vì
...
function scoreContentSig(ev) {
  return [ev.studentId,ev.week,ev.title,ev.points,ev.type].join('|');
}
...
async function loadScoreboardData(force=false) {
  ...
  if(cached) {
    ...
    setState({dataSource:DATA_SOURCE.LOADING,syncMessage:''});
  }
...
function applyRemoteData(remote, opts={}) {
  ...
  const now = Date.now();
  if(opts.notify&&state.dataSource===DATA_SOURCE.GAS&&!savingActive) {
...
function delContentKey(d) { return [d.studentId, d.week, d.title, d.points].join('|'); }
...
async function saveScoreChanges(changes) {
  ...
  // deletions: mảng {id, studentId, week, title, points} (hỗ trợ cả id-string
  // cũ để tương thích ngược) — gửi kèm NỘI DUNG để server khớp đúng dòng cần
  // xoá, thay vì chỉ dựa vào id sinh theo vị trí (dễ lệch nếu dòng bị ghi lại).
  ...
  // Chặn gửi trùng: nếu đúng nội dung này đang được gửi lên Sheets rồi (chưa
  // có phản hồi) mà người dùng bấm Lưu lần nữa — KHÔNG gửi thêm 1 request
  // POST nữa, vì request trước có thể vẫn đang xử lý thành công phía server,
  // gửi lại sẽ tạo dòng trùng trong Google Sheets.
  ...
  if(savingActive && signature===lastSaveSignature && (Date.now()-lastSaveAt)<SAVE_DEDUPE_WINDOW_MS) {
    setState({syncMessage:'Yêu cầu này đang được gửi rồi, vui lòng đợi vài giây...'});
    return;
  }
  ...
  // Snapshot số lượng theo nội dung TRƯỚC khi lưu — cần để về sau xác nhận
  // "đã xoá đúng số lượng" mà KHÔNG dựa vào id (id sẽ bị đánh lại theo vị
  // trí dòng một khi 1 dòng phía trước trong cùng ô bị xoá).
  const beforeDelCounts = countByDelKey(prevEvents);
  pendingSaveGuard = { additions:optimistic, deletions:deletionDescriptors, beforeDelCounts, expiresAt:Date.now()+SAVE_GUARD_MS };
  setState({events:nextEvents, syncMessage:''});
  if(state.dataSource!==DATA_SOURCE.GAS) return;
  savingActive=true;
  try {
    const body = JSON.stringify({action:'saveScoreChanges',additions:optimistic,deletions:deletionDescriptors});
    ...
    // QUAN TRỌNG: nếu backend trả lỗi (vd. không tìm thấy học sinh/sheet tuần,
    // Lock timeout...) thì doPost() trong api.gs trả {ok:false, error:...} và
    // KHÔNG có `data`. Trước đây code chỉ kiểm tra `json?.data` nên trường hợp
    // này bị bỏ qua hoàn toàn — không có lỗi nào hiện ra, ô điểm optimistic vẫn
    // hiển thị "như đã lưu" cho tới khi pendingSaveGuard hết hạn sau 45s
    // (SAVE_GUARD_MS), lúc đó bản dữ liệu thật (chưa hề đổi) mới được áp lại
    // và người dùng thấy điểm "tự nhiên" biến mất/trả về như cũ mà không rõ
    // vì sao. Ném lỗi ngay ở đây để rơi vào catch() bên dưới — vốn đã có sẵn
    // logic hoàn tác + hiển thị thông báo lỗi tức thì.
    if (json?.ok === false) {
    ...
    // Server báo có yêu cầu xoá KHÔNG khớp được dòng nào trong Sheet (dữ liệu
    // đã đổi khác so với lúc thao tác). Báo ngay cho người dùng thay vì để
    // guard tự hết hạn sau 45s rồi "tự nhiên" hiện lại dòng điểm đó.
    const unmatched = json?.data?.unmatchedDeletions;
    if (Array.isArray(unmatched) && unmatched.length) {
      pendingSaveGuard = null; // dữ liệu thật đã được áp ở applyRemoteData phía trên, không cần giữ guard nữa
      const names = unmatched.map(u=>`${u.title} (${u.points>0?'+':''}${u.points})`).join(', ');
      setState({ syncMessage: `Không xoá được: ${names}. Dữ liệu trên Sheet đã thay đổi so với lúc bạn thao tác — vui lòng tải lại và xoá lại.` });
    }

    setTimeout(async()=>{
      const fresh=await fetchScoreboardFromGas(true);
    ...
    setState({events:prevEvents, syncMessage: timedOut
...
/* ============================================================
   12. TOAST (thông báo trực tiếp khi có dữ liệu mới)
   ============================================================ */
          ...
          ${students.map((s,i)=>{
            ...
            return `
              <tr class="${!editable?'readonly-student-row':''} ${isCur?'current-user-row':''}">
                ...
                <td><span class="status-pill status-${statusClass(s.status)}">${s.status===SCORE_STATUS.FAIL?'CĐ':s.status}</span></td>
              </tr>`;
          }).join('')}
        </tbody>
      </table>
      ...
      <div class="score-filter-bar">
        ...
        <label class="score-filter">
          ...
          ${buildFilterSelect('statusFilter',state.statusFilter,[
            ...
            {value:SCORE_STATUS.FAIR,label:'Khá'},{value:SCORE_STATUS.PASS,label:'Đạt'},{value:SCORE_STATUS.FAIL,label:'Chưa đạt'}],
...
async function createNewWeek() {
  ...
  try {
    ...
    setState({syncMessage:`Không tạo được tuần ${nextWeek} trên Google Sheets.`});
  }
...
function initScoreboard(opts={}) {
  ...
  // Đồng bộ màu chủ đạo khi desktop (cửa sổ cha) đổi theme. Không dùng màu
  // mặc định ở đây — nếu biến CSS đang tạm rỗng, giữ nguyên màu hiện có
  // thay vì ghi đè về mặc định (đúng hành vi bản gốc).
