...
async function saveScoreChanges(changes) {
  ...
  savingActive=true;
  markActivity(); // Chuyển sang polling nhanh sau khi lưu điểm
  try {
    const body = JSON.stringify({
      action:'saveScoreChanges',
      additions:optimistic,
      deletions:deletionDescriptors,
      // Danh tính thật của người đang chấm — thiếu 2 field này là lý do cột
      // "Người chỉnh sửa" trên Sheet luôn ra "Web - <giờ>" bất kể ai chấm
      // hay chấm bằng cách nào (thường/AI), vì api.gs fallback về "Web" khi
      // payload không có actorEmail/actorName.
