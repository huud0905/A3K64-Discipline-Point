...
/* ============================================================
   1) XUẤT EXCEL — port từ scoreboardExcelExport.ts
      (bản gốc gọi GAS để sinh .xlsx trên server; ở đây sinh
       .xlsx thẳng trên trình duyệt bằng SheetJS, giữ nguyên
       giao diện chọn tuần như bản gốc)
   ============================================================ */
(function excelExportModule() {
  ...
  function buildWorkbookSheetForWeek(XLSX, wb, week) {
    const payload = extraGetScoreboardPayload();
    const summaries = summarizeStudents(payload.students, payload.events, week)
      .slice()
  ...
  async function submit() {
    ...
    try {
      ...
      Array.from(picked).sort((a, b) => a - b).forEach(week => buildWorkbookSheetForWeek(XLSX, wb, week));
      const fileName = `A3K64_Export_${extraSanitizeFileName(`Tuan_${Array.from(picked).sort((a,b)=>a-b).join('_')}`)}.xlsx`;
...
(function screenshotCaptureModule() {
  ...
  function injectStyle() {
    ...
    style.textContent = `#a3-shot-hidden-area{position:fixed!important;left:-20000px!important;top:0!important;z-index:-1!important;pointer-events:none!important;opacity:1!important;background:#fff!important;width:820px!important;min-width:820px!important;overflow:visible!important}.a3-screenshot-menu{position:fixed;width:210px;background:rgba(15,23,42,.96);border:1px solid rgba(148,163,184,.28);border-radius:14px;padding:6px;z-index:2147483647;box-shadow:0 18px 50px rgba(0,0,0,.38);backdrop-filter:blur(20px);animation:a3ShotIn .16s ease}.a3-shot-dropdown-header{padding:9px 12px 8px;font-size:10px;font-weight:800;text-transform:uppercase;color:#94a3b8;letter-spacing:1px;border-bottom:1px solid rgba(148,163,184,.18);margin-bottom:5px}.a3-screenshot-menu button{width:100%;border:0;background:transparent;color:#e5e7eb;text-align:left;padding:10px 12px;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer}.a3-screenshot-menu button:hover{background:rgba(59,130,246,.14);color:#93c5fd}.a3-share-mobile{display:none!important}@media(max-width:760px){.a3-share-mobile{display:block!important}}.a3-shot-loading{position:fixed;inset:0;z-index:2147483647;background:rgba(2,6,23,.46);backdrop-filter:blur(3px);display:grid;place-items:center}.a3-shot-loading>div{min-width:245px;border:1px solid rgba(148,163,184,.24);border-radius:22px;display:grid;grid-template-columns:34px 1fr;gap:12px;align-items:center;padding:18px 20px;background:#111827;color:#f8fafc;box-shadow:0 24px 80px rgba(0,0,0,.42)}.a3-shot-loading i{width:24px;height:24px;border:3px solid rgba(59,130,246,.22);border-top-color:#3b82f6;border-radius:999px;animation:a3Spin .75s linear infinite}.a3-shot-loading strong{display:block;font-size:15px}.a3-shot-loading span{display:block;margin-top:3px;color:#94a3b8;font-size:13px}.a3-shot-preview-backdrop{position:fixed;inset:0;z-index:2147483647;background:rgba(2,6,23,.62);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:22px}.a3-shot-preview-card{width:min(1180px,96vw);height:min(900px,92vh);border:1px solid rgba(148,163,184,.28);border-radius:28px;background:#0f172a;color:#f8fafc;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 30px 100px rgba(0,0,0,.5)}.a3-shot-preview-header{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;padding:18px 22px;background:linear-gradient(135deg,rgba(37,99,235,.18),rgba(15,23,42,.98));border-bottom:1px solid rgba(148,163,184,.18)}.a3-shot-preview-header span{display:block;font-size:11px;font-weight:800;letter-spacing:3px;color:#94a3b8}.a3-shot-preview-header strong{display:block;margin-top:4px;font-size:18px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:760px}.a3-shot-preview-header small{display:block;margin-top:5px;color:#cbd5e1}.a3-shot-close{width:38px;height:38px;border:0;border-radius:13px;background:rgba(148,163,184,.13);color:#f8fafc;cursor:pointer;font-size:24px;font-weight:800}.a3-shot-preview-body{flex:1;overflow:auto;padding:18px;background:#111827;display:grid;place-items:start center}.a3-shot-preview-body img{max-width:100%;height:auto;border-radius:12px;background:#fff;box-shadow:0 18px 50px rgba(0,0,0,.35)}.a3-shot-preview-footer{display:flex;justify-content:flex-end;gap:10px;padding:14px 18px;border-top:1px solid rgba(148,163,184,.18)}.a3-shot-preview-footer button{border:0;border-radius:14px;padding:11px 16px;font-weight:800;cursor:pointer}.a3-shot-copy{background:#1e293b;color:#dbeafe}.a3-shot-download{background:#2563eb;color:#fff}.a3-shot-toast{position:fixed;right:18px;bottom:22px;z-index:2147483647;min-width:250px;max-width:min(380px,calc(100vw - 28px));border-radius:18px;padding:14px 16px;background:#111827;color:#f8fafc;border:1px solid rgba(148,163,184,.24);box-shadow:0 18px 50px rgba(0,0,0,.36);display:grid;gap:4px}.a3-shot-toast strong{font-size:14px}.a3-shot-toast span{font-size:13px;color:#cbd5e1}.a3-shot-toast.success{border-color:rgba(34,197,94,.45)}.a3-shot-toast.warning{border-color:rgba(245,158,11,.45)}.a3-shot-toast.error{border-color:rgba(239,68,68,.5)}@keyframes a3Spin{to{transform:rotate(360deg)}}@keyframes a3ShotIn{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}.theme-light .a3-screenshot-menu{background:#fff;color:#0f172a}.theme-light .a3-screenshot-menu button{color:#0f172a}.theme-light .a3-shot-preview-card{background:#fff;color:#0f172a}.theme-light .a3-shot-preview-body{background:#f1f5f9}`;
