...
/* BUG CŨ #2: cache chỉ ghi sessionStorage. Trong app desktop (Electron),
   mỗi cửa sổ (BrowserWindow) có sessionStorage RIÊNG — không chia sẻ
   giữa cửa sổ login (nơi preload.js chạy) và cửa sổ Profile, và cũng
   mất sạch mỗi khi cửa sổ Profile bị đóng rồi mở lại. Đó là lý do
   preload.js chạy xong ngay sau login nhưng Profile vẫn "load chay"
   ~3s, và bật/tắt cửa sổ Profile là load lại từ đầu mỗi lần — trong
   khi scoreboard.js/seating.js dùng localStorage nên tức thời.
   Fix: đọc/ghi localStorage (persist xuyên cửa sổ + xuyên phiên mở/
   đóng), sessionStorage + biến in-memory chỉ giữ làm lớp cache nhanh
   nhất trong cùng 1 cửa sổ. */
function readGasCache() {
  ...
  try {
    const raw = JSON.parse(localStorage.getItem(GAS_CACHE_KEY) || 'null');
    if (raw && Date.now() - raw.ts < GAS_CACHE_TTL_MS) { gasMemCache = raw; return raw.data; }
  } catch {}
  return null;
}
function writeGasCache(data) {
  ...
  try { localStorage.setItem(GAS_CACHE_KEY, JSON.stringify(gasMemCache)); } catch {}
}
