...
const Duty = {
  ...
  // v9 — Tab Quản lý: điều hướng con (sub-tab) để đỡ phải cuộn qua tất cả mục cùng lúc
  manageSection: 'debt', // 'debt' | 'config' | 'danger'
};
...
/* v8 — Làm mới Duty.weekGroup/outdoorGroup VÀ classSlots/outdoorSlots từ
   server (không đụng exemptList/skipDays/render). Dùng để đồng bộ lại trạng
   thái "Tổ nào đang phụ trách khu vực nào, có slots thật hay không" trước khi
   mở Wizard (quyết định khoá khu vực), và sau khi Wizard báo lỗi — vì
   saveDutyWeekGroup/saveDutyOutdoorWeekGroup có thể đã lưu THÀNH CÔNG vào DB
   dù bước sau đó (vd generateDutyRoster) lỗi giữa chừng. Nếu không đồng bộ
   lại, biến local có thể lệch với server (vd sau khi Xoá lịch ở tab khác). */
async function dutyRefreshGroupState() {
  try {
    const [data, outdoorData] = await Promise.all([
      dutyApiGet('getDutyRoster', { week: Duty.week }),
      dutyApiGet('getDutyOutdoorRoster', { week: Duty.week }),
    ...
    // Không chặn luồng chính nếu refresh lỗi — chỉ log, giữ nguyên giá trị cũ.
    console.error('[duty] Không làm mới được trạng thái Tổ phụ trách:', err);
  }
}
...
function dutyRenderToday() {
  ...
  const classHasRoster   = (Duty.classSlots   || []).length > 0;
  const outdoorHasRoster = (Duty.outdoorSlots || []).length > 0;
  /* Hero section — chỉ hiện khi THẬT SỰ có lịch (slots), không chỉ vì còn
     lưu cấu hình Tổ (weekGroup có thể còn sau khi bấm "Xoá lịch" — slots giữ
     rỗng, cấu hình vẫn giữ để bấm "Khởi tạo" lại). Nếu chỉ còn cấu hình mà
     chưa có slots, coi như CHƯA có lịch — tránh hiện lầm "đang có lịch". */
  const heroHtml = (Duty.weekGroup && classHasRoster) ? `
    <div class="dt-hero">
  ...
  ` : (!classHasRoster && !outdoorHasRoster ? `
  ...
  const outdoorHeroHtml = (Duty.outdoorGroup && outdoorHasRoster) ? `
...
function dutyRenderWeek() {
  ...
  const canEdit = dutyCanEdit();
  const classTable = classRows.length ? dutyBuildWeekTable(classRows, Duty.workItems || [], canEdit) : '';
  const outdoorTable = outdoorRows.length ? dutyBuildWeekTable(outdoorRows, Duty.outdoorWorkItems || [], canEdit) : '';
  // v10 — 2 bảng đặt song song tỉ lệ 3/5 (trong lớp) : 2/5 (sân trường) trên
  // màn hình rộng; tự xuống dòng (stack) trên màn hình hẹp — xem CSS .dw-week-grid.
  body.innerHTML = `
    <div class="dw-week-grid">
      <div class="dw-table-wrap dw-area-class">
        <div class="dm-section-title" style="margin-bottom:8px;">🏠 Trong lớp${Duty.weekGroup?` — Tổ ${dutyEscape(String(Duty.weekGroup))}`:''}</div>
        <div class="dw-table-scroll"><table class="dw-table">${classTable}</table></div>
      </div>
      ${outdoorTable ? `
      <div class="dw-table-wrap dw-area-outdoor">
        ...
        <div class="dw-table-scroll"><table class="dw-table">${outdoorTable}</table></div>
      </div>` : ''}
    </div>`;
  // Tái dùng bộ bind sự kiện của tab Hôm nay — cùng cơ chế sửa phân công
  // (data-edit-cell / data-save-edit / data-cancel-edit) nên chạy được luôn,
  // không cần viết lại logic lưu/huỷ riêng cho bảng tuần.
  dutyBindTodayEvents(canEdit);
  dutyBindWeekQuickRemove();
}

// v10 — Xoá nhanh 1 người khỏi phân công ngay trên chip tên (không cần mở
// cả form "Sửa phân công"). Chỉ xoá đúng người đó, giữ nguyên những người còn lại.
function dutyBindWeekQuickRemove() {
  const body = document.getElementById('da-body');
  if (!body) return;
  body.querySelectorAll('[data-quick-remove]').forEach(btn => {
    btn.onclick = async (e) => {
      e.stopPropagation();
      const [cellId, studentId, studentName] = btn.dataset.quickRemove.split('|');
      if (!confirm(`Xoá ${studentName} khỏi phân công này?`)) return;
      const slot = Duty.classSlots.flatMap(r => r.items).find(it => it.id === cellId)
        || Duty.outdoorSlots.flatMap(r => r.items).find(it => it.id === cellId);
      if (!slot) return;
      const assignedStudents = (slot.assignedStudents || [])
        .filter(s => s.id !== studentId)
        .map(s => ({ id: s.id, name: s.name }));
      btn.disabled = true;
      try {
        const result = await dutyApiPost('saveDutyOverride', { id: cellId, assignedStudents });
        if (result?.ok === false) throw new Error(result.error || 'Backend từ chối yêu cầu.');
        await dutyLoadClass();
        dutyNotify(`Đã xoá ${studentName} khỏi phân công.`, 'success');
      } catch(err) { dutyNotify('Không xoá được: ' + (err?.message||err), 'error'); }
    };
  });
}

function dutyBuildWeekTable(rows, items, canEdit) {
  const todayLabel = dutyTodayDayLabel();
  const currentUser = readDutyUser();
  const meId = currentUser?.id || currentUser?.studentId || '';
  const colSpan = 1 + items.length;
  ...
  const tbody = rows.map(row => {
    ...
    // v10 — mỗi hàng có thể mở 1 form sửa phân công ngay bên dưới (full-width),
    // dùng chung dutyRenderCellEditForm với tab Hôm nay để đổi/xoá người trực tiếp.
    let editRowHtml = '';
    const cells = items.map(itemDef => {
      const cell = row.items.find(it => it.workItemId === itemDef.id);
      if (!cell) return `<td><span class="dw-empty-cell">—</span></td>`;
      const isEditingThis = canEdit && Duty.editingCell?.rowIndex === cell.id && Duty.editingCell?.workItemId === itemDef.id;
      if (isEditingThis) {
        editRowHtml = `<tr class="dw-edit-row"><td colspan="${colSpan}">${dutyRenderCellEditForm(row, cell, itemDef)}</td></tr>`;
      }
      const students = cell.assignedStudents || [];
      const names = students.length ? `<div class="dw-names">${students.map(s => {
        const isMe = meId && String(s.id) === String(meId);
        const tag = s.penalty === 'absent'
          ? `<span class="dt-penalty-tag bo">Bỏ</span>`
          : s.penalty === 'late'
            ? `<span class="dt-penalty-tag muon">Muộn</span>`
            : '';
        const quickX = canEdit ? `<button class="dw-chip-x" data-quick-remove="${dutyEscape(cell.id)}|${dutyEscape(s.id)}|${dutyEscape(s.name)}" title="Xoá ${dutyEscape(s.name)} khỏi phân công">×</button>` : '';
        return `<span class="dw-name-chip${s.penalty?' penalised':''}${isMe?' is-me':''}" ${isMe?'title="Bạn"':''}>${dutyEscape(s.name)}${tag}${quickX}</span>`;
      }).join('')}</div>` : `<span class="dw-empty-cell">Chưa phân công</span>`;
      const doneChip = dutyIsDone(cell.status) ? `<span class="da-chip done" style="font-size:9.5px;">✓</span>` : '';
      const editBtn = canEdit ? `<button class="dw-cell-edit-btn" data-edit-cell="${dutyEscape(cell.id)}|${dutyEscape(itemDef.id)}" title="Đổi / xoá người trực">✎</button>` : '';
      const cellHead = (doneChip || editBtn) ? `<div class="dw-cell-head">${doneChip}${editBtn}</div>` : '';
      return `<td class="${isEditingThis?'is-editing':''}">${cellHead}${names}</td>`;
    }).join('');
    return `<tr${isToday?' style="background:color-mix(in srgb,var(--accent) 5%,var(--surface))"':''}>${dayCell}${cells}</tr>${editRowHtml}`;
  }).join('');
...
function dutyRenderManage() {
  ...
  // v9 — người không có quyền sửa chỉ xem được Sổ nợ, không cần sub-tab
  if (!canEdit) Duty.manageSection = 'debt';
  const section = Duty.manageSection || 'debt';
  ...
  const owingBadge = allDebts.length;

  let html = `<div class="dm-layout">`;

  /* ── Thanh trạng thái Tổ trực tuần — luôn hiển thị, không phụ thuộc sub-tab ── */
  html += `
    <div class="dm-status-strip">
      <div class="dm-status-chip">
        <span class="dm-status-icon">🏠</span>
        <span class="dm-status-text">Trong lớp: ${Duty.weekGroup ? `<strong>Tổ ${dutyEscape(String(Duty.weekGroup))}</strong>` : '<span class="dm-status-empty">chưa cấu hình</span>'}</span>
      </div>
      <div class="dm-status-chip">
        <span class="dm-status-icon">🏫</span>
        <span class="dm-status-text">Sân trường: ${Duty.outdoorGroup ? `<strong>Tổ ${dutyEscape(String(Duty.outdoorGroup))}</strong>` : '<span class="dm-status-empty">chưa tách riêng</span>'}</span>
      </div>
    </div>`;

  /* ── Sub-tabs điều hướng (chỉ hiện khi có quyền sửa — người thường chỉ có Sổ nợ) ── */
  if (canEdit) {
    html += `
    <div class="dm-subtabs">
      <button class="dm-subtab ${section==='debt'?'active':''}" data-manage-section="debt">Sổ nợ${owingBadge?` <span class="dm-subtab-badge">${owingBadge}</span>`:''}</button>
      <button class="dm-subtab ${section==='config'?'active':''}" data-manage-section="config">Cấu hình</button>
      <button class="dm-subtab danger ${section==='danger'?'active':''}" data-manage-section="danger">⚠ Nguy hiểm</button>
    </div>`;
  }
  ...
  /* ══ SECTION: Cấu hình (chỉ tổ trưởng) ══ */
  if (section === 'config' && canEdit) {
    const swapReqCount = (Duty.swapRequests||[]).length;
    const swapMkCount = (Duty.swapMakeups||[]).filter(m=>m.status==='pending').length;
    const overrideCount = (Duty.overrides||[]).length;
    html += `
      <div class="dm-cfg-group">
        <div class="dm-cfg-group-title">🗓 Khởi tạo lịch trực</div>
        <div style="display:flex;flex-direction:column;gap:6px;">
          ${dutyRenderAccordion('week-config', '🏠 Khởi tạo lịch tuần (trong lớp)', 'Tổ trực · Xung kích', Duty.weekConfigOpen, Duty.weekConfigOpen ? dutyRenderWeekConfigPanel() : '')}
        ...
        ${Duty.outdoorGroup
          ? `<p class="da-form-hint" style="margin-top:8px;">🏫 Tổ ${dutyEscape(String(Duty.outdoorGroup))} đang phụ trách sân trường tuần ${Duty.week} — lịch tự sinh song song với lớp, xem &amp; sửa tay ở tab "Hôm nay" / "Tuần này".</p>`
          : `<p class="da-form-hint" style="margin-top:8px;">Cần ít nhất 2 Tổ để tách riêng sân trường khỏi lớp.</p>`}
      </div>

      <div class="dm-cfg-group">
        <div class="dm-cfg-group-title">📋 Đầu việc</div>
        <div style="display:flex;flex-direction:column;gap:6px;">
          ${dutyRenderAccordion('workitems-config', 'Đầu việc trong lớp', `${(Duty.workItems||[]).length} việc`, Duty.workItemsConfigOpen, Duty.workItemsConfigOpen ? dutyRenderWorkItemsPanel() : '')}
          ${dutyRenderAccordion('outdoor-workitems-config', 'Đầu việc sân trường', `${(Duty.outdoorWorkItems||[]).length} việc`, Duty.outdoorWorkItemsConfigOpen, Duty.outdoorWorkItemsConfigOpen ? dutyRenderOutdoorWorkItemsPanel() : '')}
        </div>
      </div>

      <div class="dm-cfg-group">
        <div class="dm-cfg-group-title">🔄 Đổi ca &amp; vắng mặt</div>
        <div style="display:flex;flex-direction:column;gap:6px;">
          ${dutyRenderAccordion('swap-requests', 'Yêu cầu đổi ca', `${swapReqCount} yêu cầu`, Duty.swapRequestsOpen, Duty.swapRequestsOpen ? dutyRenderSwapRequestsPanel() : '')}
          ${dutyRenderAccordion('swap-makeups', 'Đang chờ bù ca', `${swapMkCount} pending`, Duty.swapMakeupsOpen, Duty.swapMakeupsOpen ? dutyRenderSwapMakeupsPanel() : '')}
          ${dutyRenderAccordion('override-config', 'Ghi đè theo ngày/tuần', `${overrideCount} ghi đè`, Duty.overrideOpen, Duty.overrideOpen ? dutyRenderOverridePanel() : '')}
        </div>
      </div>

      <div class="dm-cfg-group">
        <div class="dm-cfg-group-title">⚙ Khác</div>
        <div style="display:flex;flex-direction:column;gap:6px;">
  ...
  /* ══ SECTION: Vùng nguy hiểm (chỉ tổ trưởng) ══ */
  /* v8 — tách riêng theo khu vực (trong lớp / sân trường): trước đây 1 nút
     "Xoá lịch"/"Xoá sạch" xoá CẢ HAI khu vực cùng lúc, dễ xoá nhầm Tổ của
     khu vực còn lại khi người dùng chỉ định xử lý 1 khu vực. Giờ mỗi khu vực
     có nút riêng, chỉ tác động tới đúng khu vực đó (backend cần đọc payload
     "area": "class" | "outdoor" ở action clearDutyRoster). */
  if (section === 'danger' && canEdit) {
    const hasClassRoster = (Duty.classSlots||[]).length > 0;
    const hasOutdoorRoster = (Duty.outdoorSlots||[]).length > 0;
    html += `
    <div class="dm-danger-zone">
      <p class="da-form-hint">Các thao tác dưới đây <strong>không thể hoàn tác</strong>. Chỉ dùng khi cần thiết. Trong lớp và sân trường tách riêng — xoá bên nào không ảnh hưởng tới Tổ &amp; lịch của bên còn lại.</p>

      <div class="dm-danger-group">
        <div class="dm-danger-group-title">🏠 Trong lớp</div>
        <div style="display:flex;flex-direction:column;gap:10px;margin-top:8px;">
          <div class="dm-danger-row">
            <div>
              <div class="dm-danger-label">Xoá lịch tuần ${Duty.week} — trong lớp</div>
              <div class="dm-danger-desc">Xoá slots lịch trực trong lớp tuần này. Giữ nguyên cấu hình Tổ trong lớp — bấm "Khởi tạo lịch tuần" lại để tạo mới. Sân trường không bị ảnh hưởng.</div>
            </div>
            <button class="da-btn danger" id="dm-clear-roster-class" ${!hasClassRoster?'disabled title="Tuần này chưa có lịch trong lớp"':''}>Xoá lịch</button>
          </div>
          <div class="dm-danger-row">
            <div>
              <div class="dm-danger-label">Xoá sạch tuần ${Duty.week} — trong lớp</div>
              <div class="dm-danger-desc">Xoá cả lịch lẫn cấu hình Tổ trực trong lớp tuần này. Phải cấu hình lại từ đầu. Sân trường không bị ảnh hưởng.</div>
            </div>
            <button class="da-btn danger" id="dm-clear-all-class" ${!hasClassRoster?'disabled title="Tuần này chưa có lịch trong lớp"':''}>Xoá sạch</button>
          </div>
        </div>
      </div>

      <div class="dm-danger-group">
        <div class="dm-danger-group-title">🏫 Sân trường</div>
        <div style="display:flex;flex-direction:column;gap:10px;margin-top:8px;">
          <div class="dm-danger-row">
            <div>
              <div class="dm-danger-label">Xoá lịch tuần ${Duty.week} — sân trường</div>
              <div class="dm-danger-desc">Xoá slots lịch trực sân trường tuần này. Giữ nguyên cấu hình Tổ sân trường — bấm "Khởi tạo lịch sân trường" lại để tạo mới. Trong lớp không bị ảnh hưởng.</div>
            </div>
            <button class="da-btn danger" id="dm-clear-roster-outdoor" ${!hasOutdoorRoster?'disabled title="Tuần này chưa có lịch sân trường"':''}>Xoá lịch</button>
          </div>
          <div class="dm-danger-row">
            <div>
              <div class="dm-danger-label">Xoá sạch tuần ${Duty.week} — sân trường</div>
              <div class="dm-danger-desc">Xoá cả lịch lẫn cấu hình Tổ trực sân trường tuần này. Phải cấu hình lại từ đầu. Trong lớp không bị ảnh hưởng.</div>
            </div>
            <button class="da-btn danger" id="dm-clear-all-outdoor" ${!hasOutdoorRoster?'disabled title="Tuần này chưa có lịch sân trường"':''}>Xoá sạch</button>
          </div>
        </div>
      </div>
    </div>`;
...
function dutyBindManageEvents(canEdit) {
  const body = document.getElementById('da-body');
  if (!body) return;
  /* Sub-tabs — v9: chuyển giữa Sổ nợ / Cấu hình / Nguy hiểm mà không cuộn cả trang */
  body.querySelectorAll('[data-manage-section]').forEach(btn => {
    btn.addEventListener('click', () => {
      Duty.manageSection = btn.dataset.manageSection;
      dutyRenderBody();
    });
  });
  /* Danger zone — v8: tách riêng theo khu vực, mỗi nút chỉ gửi payload
     với "area" tương ứng và chỉ reset state phía đúng khu vực đó. */
  document.getElementById('dm-clear-roster-class')?.addEventListener('click', async () => {
    if (!confirm(`Xoá lịch trực TRONG LỚP tuần ${Duty.week}?\n\nCấu hình Tổ trong lớp vẫn được giữ. Sân trường không bị ảnh hưởng. Bấm "Khởi tạo lịch tuần" để tạo lại.`)) return;
    const btn = document.getElementById('dm-clear-roster-class');
    if (btn) { btn.disabled = true; btn.textContent = 'Đang xoá…'; }
    try {
      await dutyApiPost('clearDutyRoster', { week: Duty.week, area: 'class', clearConfig: false });
      Duty.classLoaded = false; Duty.classSlots = []; Duty.weekGroup = null; Duty.exemptList = [];
      dutyNotify(`Đã xoá lịch trong lớp tuần ${Duty.week}. Cấu hình Tổ vẫn còn, bấm "Khởi tạo" để tạo lại.`, 'success');
      dutyRenderBody();
    } catch(err) { dutyNotify('Không xoá được: ' + (err?.message||err), 'error'); if (btn) { btn.disabled = false; btn.textContent = 'Xoá lịch'; } }
  });
  document.getElementById('dm-clear-all-class')?.addEventListener('click', async () => {
    if (!confirm(`Xoá SẠCH lịch & cấu hình TRONG LỚP tuần ${Duty.week}?\n\nSân trường không bị ảnh hưởng. Phải cấu hình lại từ đầu.`)) return;
    const btn = document.getElementById('dm-clear-all-class');
    if (btn) { btn.disabled = true; btn.textContent = 'Đang xoá…'; }
    try {
      await dutyApiPost('clearDutyRoster', { week: Duty.week, area: 'class', clearConfig: true });
      Duty.classLoaded = false; Duty.classSlots = []; Duty.weekGroup = null; Duty.exemptList = [];
      Duty.weekConfigDraft = null;
      dutyNotify(`Đã xoá sạch lịch & cấu hình trong lớp tuần ${Duty.week}.`, 'success');
      dutyRenderBody();
    } catch(err) { dutyNotify('Không xoá được: ' + (err?.message||err), 'error'); if (btn) { btn.disabled = false; btn.textContent = 'Xoá sạch'; } }
  });
  document.getElementById('dm-clear-roster-outdoor')?.addEventListener('click', async () => {
    if (!confirm(`Xoá lịch trực SÂN TRƯỜNG tuần ${Duty.week}?\n\nCấu hình Tổ sân trường vẫn được giữ. Trong lớp không bị ảnh hưởng. Bấm "Khởi tạo lịch sân trường" để tạo lại.`)) return;
    const btn = document.getElementById('dm-clear-roster-outdoor');
    if (btn) { btn.disabled = true; btn.textContent = 'Đang xoá…'; }
    try {
      await dutyApiPost('clearDutyRoster', { week: Duty.week, area: 'outdoor', clearConfig: false });
      Duty.outdoorSlots = []; Duty.outdoorGroup = null;
      dutyNotify(`Đã xoá lịch sân trường tuần ${Duty.week}. Cấu hình Tổ vẫn còn, bấm "Khởi tạo" để tạo lại.`, 'success');
      dutyRenderBody();
    } catch(err) { dutyNotify('Không xoá được: ' + (err?.message||err), 'error'); if (btn) { btn.disabled = false; btn.textContent = 'Xoá lịch'; } }
  });
  document.getElementById('dm-clear-all-outdoor')?.addEventListener('click', async () => {
    if (!confirm(`Xoá SẠCH lịch & cấu hình SÂN TRƯỜNG tuần ${Duty.week}?\n\nTrong lớp không bị ảnh hưởng. Phải cấu hình lại từ đầu.`)) return;
    const btn = document.getElementById('dm-clear-all-outdoor');
    if (btn) { btn.disabled = true; btn.textContent = 'Đang xoá…'; }
    try {
      await dutyApiPost('clearDutyRoster', { week: Duty.week, area: 'outdoor', clearConfig: true });
      Duty.outdoorSlots = []; Duty.outdoorGroup = null;
      dutyNotify(`Đã xoá sạch lịch & cấu hình sân trường tuần ${Duty.week}.`, 'success');
      dutyRenderBody();
    } catch(err) { dutyNotify('Không xoá được: ' + (err?.message||err), 'error'); if (btn) { btn.disabled = false; btn.textContent = 'Xoá sạch'; } }
  });
  /* Penalty form toggle */
...
async function dutyOpenWizard() {
  await Promise.all([dutyLoadWorkItems(), dutyLoadOutdoorWorkItems(), dutyLoadStudents()]);
  // v7 — luôn lấy lại Tổ đang phụ trách mỗi khu vực TỪ SERVER trước khi tính
  // khoá (lockedAreas/usedByClass/usedByOutdoor), tránh dùng biến local đã cũ.
  await dutyRefreshGroupState();
  if (!Duty.skipDaysLoaded) {
    try { Duty.skipDays = await dutyApiGet('getDutySkipDays', { week: Duty.week }) || []; }
    catch (_) { Duty.skipDays = []; }
    Duty.skipDaysLoaded = true;
  }
  // Khu vực đã tạo lịch xong (CÓ SLOTS THẬT, không chỉ còn lưu cấu hình Tổ)
  // → khoá lại, chỉ mở khu vực chưa có lịch. weekGroup/outdoorGroup có thể
  // vẫn còn dù slots đã bị xoá (vd bấm "Xoá lịch" — giữ cấu hình để tạo lại
  // nhanh) — trường hợp đó KHÔNG được tính là "đã tạo", để Wizard vẫn cho
  // chọn lại bình thường thay vì khoá cứng, phải vòng qua tab Quản lý.
  ...
  const outdoorHasRoster = (Duty.outdoorSlots || []).length > 0;
  const lockedAreas = [];
  if (Duty.outdoorGroup && outdoorHasRoster) lockedAreas.push('outdoor');
  if (Duty.weekGroup    && classHasRoster)   lockedAreas.push('class');
...
async function dutyWizardSubmit() {
  ...
  try {
    ...
    // v7 — dù lỗi ở bước nào (vd generateDutyRoster), các lệnh lưu Tổ trước đó
    // (saveDutyWeekGroup / saveDutyOutdoorWeekGroup) có thể đã lưu THÀNH CÔNG
    // vào DB rồi. Đồng bộ lại ngay để nếu người dùng thử lại khu vực còn lại
    // (cùng phiên hoặc mở lại Wizard), Tổ đã dùng sẽ được khoá đúng — không bị
    // gán trùng 1 Tổ cho cả 2 khu vực.
    await dutyRefreshGroupState();
    if (Duty.wizardDraft) {
      const d = Duty.wizardDraft;
      if (Duty.outdoorGroup) { d.usedByOutdoor = Number(Duty.outdoorGroup); if (!d.lockedAreas.includes('outdoor')) d.lockedAreas.push('outdoor'); }
      if (Duty.weekGroup)    { d.usedByClass   = Number(Duty.weekGroup);    if (!d.lockedAreas.includes('class'))   d.lockedAreas.push('class'); }
    }
  } finally {
