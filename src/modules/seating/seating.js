...
function loadSeatState() {
  // Stale-while-revalidate: hiện ngay dữ liệu đã cache từ lần trước (DB local),
  // boot() sẽ fetch GAS ngầm và cập nhật nếu có thay đổi.
  // Chỉ dùng lưới trống khi chưa từng có sơ đồ nào được lưu.
  try {
    const savedId = localStorage.getItem(SEAT_CURRENT_KEY);
    const items = readDb();
    const cached = savedId
      ? items.find(i => i.id === savedId) || items.find(i => i.active) || items[0]
      : items.find(i => i.active) || items[0];
    if (cached?.layout?.seats) {
      const seats = cached.layout.seats;
      if (seats.left && seats.right) {
        return { left: normalizeRows(seats.left), right: normalizeRows(seats.right) };
      }
    }
  } catch (e) { /* ignore */ }
  return cloneState(SEAT_DEFAULT);
}
...
/** Ghi layout (seats+room) đầy đủ của 1 sơ đồ vào DB nhiều-sơ-đồ (SEAT_DB_KEY).
 *
 * Trong chế độ gas (dataSource==='gas'), SEAT_DB_KEY trước giờ chỉ được ghi
 * bởi ensureDb() (seed mặc định lúc chưa có sơ đồ nào) và bởi preload.js
 * (chỉ có metadata từ listSeatingCharts — KHÔNG có layout.seats). Nghĩa là
 * layout thật chưa từng được lưu vào DB nhiều-sơ-đồ này — mỗi lần
 * preload.js chạy lại (login/reload), nó merge dựa trên layout cũ trong
 * DB, nhưng DB chưa bao giờ có layout để mà giữ lại → sơ đồ luôn phải chờ
 * gasGetChart() xong mới có gì để hiện.
 *
 * Gọi hàm này mỗi khi vừa fetch được layout thật từ gasGetChart() (boot(),
 * applySheet(), checkAccessNow()) để lần preload/reload sau, DB đã có sẵn
 * layout mà preload.js có thể giữ lại khi merge. */
function persistLayoutToDb(id, seats, room, metaHint) {
  if (!id || !seats) return;
  const items = readDb();
  let item = items.find(i => i.id === id);
  const now = new Date().toISOString();
  if (!item) {
    item = Object.assign({ id, title: metaHint?.title || '', active: true, createdAt: now, updatedAt: now }, metaHint || {});
    items.push(item);
  }
  item.layout = { seats, room: room || item.layout?.room || cloneRoom(SEAT_DEFAULT_ROOM), exportedAt: now };
  item.updatedAt = now;
  writeDb(items);
}
...
let userGroup  = null;    // số tổ của người đang đăng nhập (từ desktop session)
...
// - status='published'/'public'   → tất cả xem được.
// - status='preview'/'restricted' → chỉ những ai trong previewStudents.
// - status='private'              → chỉ GVCN/lớp trưởng (đã bắt ở trên).
//
// Worker (Cloudflare) dùng: 'private' | 'restricted' | 'public'
// UI frontend dùng:         'private' | 'preview'    | 'published'
// Hai helper dưới đây dịch qua lại — mọi chỗ so sánh dùng CANONICAL_STATUS
// (giá trị UI) nhờ seatStatusFromApi(); mọi chỗ gửi lên backend dùng
// seatStatusToApi() để worker không fallback về 'private'.

/** UI-label → API-value (gửi lên worker) */
function seatStatusToApi(uiStatus) {
  if (uiStatus === 'published') return 'public';
  if (uiStatus === 'preview')   return 'restricted';
  return 'private'; // 'private' giữ nguyên
}

/** API-value → UI-label (nhận từ worker) */
function seatStatusFromApi(apiStatus) {
  if (apiStatus === 'public')      return 'published';
  if (apiStatus === 'restricted')  return 'preview';
  return 'private'; // 'private' hoặc giá trị lạ
}
...
async function gasGetChart(id) {
  ...
  const rawStatus = json?.status || data.status || '';
  return { chart: data.chart || null, restricted: !!(json?.restricted || data.restricted), status: seatStatusFromApi(rawStatus) };
}
...
async function gasGetAccess(chartId, chartTitle) {
  ...
  const acc = data.access || { status: 'private' };
  return { ...acc, status: seatStatusFromApi(acc.status || 'private') };
}
// Gộp getSeatingChart + getSeatingAccess thành 1 request duy nhất.
// Backend trả { chart, access, restricted?, status? } — giảm từ 2 GAS call
// (~6-8s) xuống còn 1 call (~3-4s). Fallback về 2 call riêng nếu backend
// chưa hỗ trợ action này (data.chart === undefined && data.access === undefined).
async function gasGetChartWithAccess(id, chartTitle) {
  const json = await fetchFromGas({ action: 'getSeatingChartWithAccess', id: id || '', chartTitle: chartTitle || '', role: userRole || '' });
  const data = gasData(json);
  // Nếu backend chưa hỗ trợ action mới → trả null để caller fallback
  if (data.chart === undefined && data.access === undefined && !data.restricted) return null;
  const rawAccess = data.access || { status: 'private' };
  const rawStatus = json?.status || data.status || '';
  return {
    chart: data.chart || null,
    access: { ...rawAccess, status: seatStatusFromApi(rawAccess.status || 'private') },
    restricted: !!(json?.restricted || data.restricted),
    status: seatStatusFromApi(rawStatus),
  };
}
async function gasSaveAccess(chartId, chartTitle, status, previewStudents) {
  const json = await postToGas('saveSeatingAccess', {
    chartId: chartId || '', chartTitle: chartTitle || '', status: seatStatusToApi(status),
    previewStudents: previewStudents || '',
    role: userRole || '', actor: { name: userName, role: userRole },
  });
  const data = gasData(json);
  const rawAcc = data.access || null;
  const normAcc = rawAcc ? { ...rawAcc, status: seatStatusFromApi(rawAcc.status || 'private') } : null;
  return { access: normAcc, error: json?.error || data.error || null };
}
...
// Mobile: gộp toàn bộ .seat-tools (chọn sơ đồ, tìm kiếm, bật sửa, icon
...
<div class="seat-app">
  <!-- ── Toolbar ── -->
  <div class="seat-toolbar${mobileToolsOpen ? ' mobile-tools-open' : ''}">
    <div class="seat-heading">
      ...
      <!-- Chỉ hiện trên mobile (CSS) — gộp toàn bộ .seat-tools vào 1 drawer -->
      <button id="btn-mobile-tools-toggle" class="seat-mobile-tools-toggle" title="Công cụ & cài đặt" aria-expanded="${mobileToolsOpen}">
        ${seatIcon('settings', 18)}
        <span>Công cụ</span>
        ${seatIcon('chevronDown', 14)}
      </button>
    </div>
  ...
  <div class="seat-main${sidebarCollapsed ? ' sidebar-collapsed' : ''}">
    ...
    <!-- Cột phải: hint + board, stack dọc khi portrait -->
    <div class="seat-board-col">
    <!-- Gợi ý xoay ngang — chỉ hiện khi mobile portrait (CSS @media),
         hiện dạng dải nhỏ phía trên board chứ không thay thế board nữa -->
    <div class="seat-rotate-hint">
      ${seatIcon('rotateCcw', 16)}
      <p><strong>Gợi ý:</strong> Xoay ngang để xem rõ hơn — hoặc cuộn ngang bên dưới.</p>
    </div>
...
function printBuildHtml(seats, changeNo, dateText) {
  ...
  // ── Vị trí cửa sổ / cửa: LẤY TỪ roomConfig HIỆN TẠI (đồng bộ với sơ đồ
  // trên màn hình — kể cả khi người dùng đã "Bật sửa" rồi kéo/đổi bên).
  // Quy đổi px→mm theo đúng tỉ lệ mặc định (26mm/90px cho cửa sổ,
  // 34mm/108px cho cửa) để giữ nguyên tỉ lệ hình ảnh cũ khi dùng giá trị mặc định.
  const WIN_MM_PER_PX  = 26 / 90;
  const DOOR_MM_PER_PX = 34 / 108;

  function windowDivs(side) {
    const list = (roomConfig.windows && roomConfig.windows[side]) || [];
    return list.map(w => {
      const top    = clampNum(w.top, 2, 96, 10);
      const height = clampNum(w.height, 30, 260, 90);
      const heightMm = (height * WIN_MM_PER_PX).toFixed(2);
      return `<div class="window ${side}" style="top:${top}%;height:${heightMm}mm;"></div>`;
    }).join('');
  }

  const doorCfg    = roomConfig.door || SEAT_DEFAULT_ROOM.door;
  const doorSide   = doorCfg.side === 'left' ? 'left' : 'right';
  const doorTop    = clampNum(doorCfg.top, 2, 96, 74);
  const doorHeight = clampNum(doorCfg.height, 30, 260, 108);
  const doorHeightMm = (doorHeight * DOOR_MM_PER_PX).toFixed(2);
  const doorDiv = `<div class="door ${doorSide}" style="top:${doorTop}%;height:${doorHeightMm}mm;"></div>`;
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .window{position:absolute;width:5mm;background:#176a8b;border:1.3px solid #08384d;}
  .window.left{left:-2.7mm;}
  .window.right{right:-2.7mm;}
  .door{position:absolute;width:5mm;background:#050505;border:1.3px solid #050505;}
  .door.left{left:-2.7mm;}
  .door.right{right:-2.7mm;}
...
function bindEvents() {
  ...
  on('btn-mobile-tools-toggle', () => { mobileToolsOpen = !mobileToolsOpen; render(); });
  ...
  if (searchEl) searchEl.addEventListener('input', e => {
    ...
    const q2 = seatNorm(query);

    // 1. Cập nhật danh sách học sinh chưa xếp trong sidebar
    const listEl = document.querySelector('.seat-student-list');
    if (listEl) {
      const unassigned = SEAT_STUDENTS.filter(n => !findSeat(seatState, n));
      const filtered   = unassigned.filter(n => !q2 || seatNorm(n).includes(q2));
      if (filtered.length === 0) {
        listEl.innerHTML = unassigned.length === 0
          ? `<div class="seat-students-note">✓ Tất cả học sinh đã có chỗ ngồi.</div>`
          : `<div class="seat-notif-empty">Không tìm thấy học sinh.</div>`;
      } else {
        const editable2 = canEditSeating();
        listEl.innerHTML = filtered.map(name => {
          const hit   = Boolean(q2 && seatNorm(name).includes(q2));
          const fullN = seatFullName(name);
          return `<div class="seat-student-card${hit ? ' highlight' : ''}" draggable="${editable2 && editMode}" data-student="${escH(name)}" title="${escH(fullN)}">
            <span>${escH(name)}</span><small>${escH(fullN !== name ? fullN : 'Chưa xếp')}</small>
          </div>`;
        }).join('');
        // Gắn lại drag cho các card vừa tạo
        listEl.querySelectorAll('[data-student]').forEach(card => {
    ...
    // 2. Cập nhật highlight trên sơ đồ (thêm/bỏ class, không tạo lại DOM)
    document.querySelectorAll('[data-side][data-row][data-seat]').forEach(cell => {
      const pos  = { side: cell.dataset.side, row: +cell.dataset.row, seat: +cell.dataset.seat };
      const name = seatAt(seatState, pos);
      const hit  = Boolean(q2 && name && seatNorm(name).includes(q2));
      cell.classList.toggle('highlight', hit);
    });
  });
  ...
  /* ── Mobile touch: cuộn + long-press xem tên đầy đủ ─────────────────────
     Vấn đề gốc:
     1. Trên touch device, browser hiểu touchstart trên element có draggable="true"
        là "muốn drag" → chặn scroll của .seat-board-wrap.
     2. :hover dính lại sau tap → full-name tooltip hiện liên tục.
     Fix:
     - Khi KHÔNG ở editMode: bỏ draggable, dùng touch-action:pan-x pan-y để
       browser tự lo scroll, không cần preventDefault().
     - Long-press (≥400ms) → thêm .show-full → CSS hiện full-name → tự xoá sau 1.5s.
  ─────────────────────────────────────────────────────────────────────────── */
  ...
  document.querySelectorAll('[data-side][data-row]').forEach(cell => {
    // Chế độ không sửa: bỏ draggable hoàn toàn để touch scroll hoạt động
    if (!editMode) {
      cell.removeAttribute('draggable');
      cell.style.touchAction = 'pan-x pan-y';
    }

    if (isTouchDevice && !editMode) {
      let pressTimer = null;
      let moved = false;
      ...
      cell.addEventListener('touchstart', e => {
        ...
        pressTimer = setTimeout(() => {
          if (!moved) {
            cell.classList.add('show-full');
            setTimeout(() => cell.classList.remove('show-full'), 1500);
          }
        }, 400);
      }, { passive: true });
  ...
  /* drag-and-drop — seat cells (chỉ gắn khi editMode) */
  if (editMode) {
  document.querySelectorAll('[data-side][data-row]').forEach(cell => {
    const pos = { side: cell.dataset.side, row: +cell.dataset.row, seat: +cell.dataset.seat };
...
async function applySheet(item) {
  ...
  if (dataSource === 'gas') {
    // Vừa fetch (hoặc đã có sẵn) layout thật cho sơ đồ này — ghi vào DB
    // nhiều-sơ-đồ để lần preload/reload sau còn giữ được.
    persistLayoutToDb(item.id, seatState, roomConfig, { title: item.title });
  }
  currentId = item.id;
  localStorage.setItem(SEAT_CURRENT_KEY, item.id);
...
async function doCreateSheet() {
  ...
  if (dataSource === 'gas') {
    ...
    persistLayoutToDb(chart.id, seatState, roomConfig, { title });
    sheets = await gasListCharts();
...
// Dùng action nhẹ "watchSeatingChart" (chỉ trả id + version + accessStatus)
// thay vì gọi "getSeatingAccess" đầy đủ — giảm tải đáng kể khi nhiều học
// sinh cùng mở sơ đồ và poll đồng thời.
// Logic xử lý:
// - Không thay đổi gì (version + accessStatus giữ nguyên) → chỉ tick đồng hồ.
// - accessStatus đổi → gọi thêm getSeatingChartWithAccess để lấy layout thật
//   + access đầy đủ (1 roundtrip), rồi áp vào UI.
let _lastWatchVersion    = 0;
let _lastWatchAccStatus  = '';

async function gasWatchChart(id) {
  const json = await fetchFromGas({ action: 'watchSeatingChart', id: id || '' });
  const data = gasData(json);
  ...
  return {
    ...
    // Worker trả accessStatus theo giá trị API ('public'/'restricted'/'private');
    // dịch sang UI-label ngay tại đây để các so sánh bên dưới nhất quán.
    accessStatus: seatStatusFromApi(data.accessStatus || 'private'),
  };
}
...
async function checkAccessNow() {
  ...
  const wasBlocked = !canViewSeating(
    publishConfig.status || 'private',
    publishConfig.previewStudents || publishConfig.preview_students || '',
  );

  // ── Bước 1: poll nhẹ — chỉ metadata ──────────────────────────────────────
  const watch = await gasWatchChart(currentId);
  if (!watch) { updateSeatCountdownText(); return; } // lỗi mạng tạm thời

  const versionUnchanged   = watch.version    === _lastWatchVersion;
  const accStatusUnchanged = watch.accessStatus === _lastWatchAccStatus;

  if (versionUnchanged && accStatusUnchanged && _lastWatchVersion !== 0) {
    // Không có gì thay đổi — chỉ tick đếm ngược, không fetch thêm.
    updateSeatCountdownText();
    return;
  }

  // ── Bước 2: có thay đổi → lấy đầy đủ (chart + access) ───────────────────
  _lastWatchVersion   = watch.version;
  _lastWatchAccStatus = watch.accessStatus;

  const title    = sheets.find(i => i.id === currentId)?.title || '';
  const combined = await gasGetChartWithAccess(currentId, title);

  let access;
  if (combined !== null) {
    // Backend hỗ trợ action gộp — 1 roundtrip
    if (combined.chart?.layout?.seats && !combined.restricted) {
      seatState = {
        left:  normalizeRows(combined.chart.layout.seats.left),
        right: normalizeRows(combined.chart.layout.seats.right),
      };
      saveSeatState(seatState);
      if (dataSource === 'gas') {
        roomConfig = normalizeRoom(combined.chart.layout.room || roomConfig);
        persistLayoutToDb(currentId, seatState, roomConfig, { title });
      }
    }
    access = combined.access;
  } else {
    // Fallback: 2 request song song (backend cũ không có action gộp)
    const [res, acc] = await Promise.all([
      gasGetChart(currentId),
      gasGetAccess(currentId, title),
    ]);
    if (res.chart?.layout?.seats && !res.restricted) {
      seatState = {
        left:  normalizeRows(res.chart.layout.seats.left),
        right: normalizeRows(res.chart.layout.seats.right),
      };
      saveSeatState(seatState);
      if (dataSource === 'gas') {
        roomConfig = normalizeRoom(res.chart.layout.room || roomConfig);
        persistLayoutToDb(currentId, seatState, roomConfig, { title });
      }
    }
    access = acc;
  ...
  if (!access) { updateSeatCountdownText(); return; }
  publishConfig = access;

  const isBlockedNow = !canViewSeating(
    publishConfig.status || 'private',
    publishConfig.previewStudents || publishConfig.preview_students || '',
  );
  ...
  if (wasBlocked && !isBlockedNow) {
    showToast(
      newStatus === 'preview'
        ? 'Bạn vừa được thêm vào danh sách xem trước.'
        : 'Sơ đồ vừa được công bố.',
      'success',
    );
    render();
    ...
    // Sơ đồ cập nhật nội dung nhưng trạng thái công bố không đổi — re-render
    // để hiện layout mới mà không cần người dùng tải lại trang.
    render();
  }
}
...
/* ---------- Boot ---------- */
async function boot() {
  // ── Bước 1: Hiện ngay cache từ lần trước (stale-while-revalidate) ──────────
  // loadSeatState() đã đọc DB local ở trên — render() hiện luôn không trắng màn
  // hình. Người dùng thấy sơ đồ gần như tức thì; GAS fetch chạy ngầm phía sau.
  render();

  if (!gasUrl) { dataSource = 'local'; return; }

  try {
    // ── Bước 2: Fetch danh sách sơ đồ (nhẹ, chỉ metadata) ──────────────────
    ...
    if (charts.length) {
      ...
      // ── Bước 3: Gộp getChart + getAccess → 1 request song song ─────────────
      // Thử action mới (getSeatingChartWithAccess) trước; nếu backend chưa hỗ
      // trợ thì fallback về 2 request song song để vẫn nhanh hơn tuần tự.
      let res, access;
      const combined = await gasGetChartWithAccess(currentId, active.title);
      if (combined !== null) {
        // Backend hỗ trợ action gộp — 1 roundtrip duy nhất
        res    = { chart: combined.chart, restricted: combined.restricted, status: combined.status };
        access = combined.access;
      } else {
        // Fallback: 2 request chạy SONG SONG (Promise.all) thay vì tuần tự
        [res, access] = await Promise.all([
          gasGetChart(currentId),
          gasGetAccess(currentId, active.title),
        ]);
      }
      ...
      if (res.chart?.layout?.seats) {
        // Ghi lại vào DB nhiều-sơ-đồ để lần preload/reload sau còn giữ được
        // (xem comment ở persistLayoutToDb).
        persistLayoutToDb(currentId, seatState, roomConfig, { title: active.title, active: true });
      }
      publishConfig = access;
      // Ghi baseline cho watchSeatingChart: poll đầu tiên sau boot() sẽ bỏ
      // qua fetch đầy đủ nếu version + accessStatus chưa thay đổi.
      _lastWatchAccStatus = publishConfig.status || 'private';
    } else {
...
function fitBoard() {
  ...
  // Mobile (≤900px): board giữ kích thước tự nhiên, wrap cuộn ngang+dọc.
  // Không scale — xoá transform cũ nếu có rồi thoát.
  ...
  // Desktop: scale board để vừa wrap (fit-to-screen), không cuộn.
  // Reset về tự nhiên trước khi đo để không bị ảnh hưởng bởi scale cũ.
  board.style.transform = '';
  board.style.transformOrigin = '';
  board.style.width  = '';
  board.style.height = '';
  ...
  // Guard: iframe/WebView chưa layout xong → 0, tránh NaN/Infinity.
  if (!wW || !bW || !wH || !bH) return;

  const scale = Math.min(wW / bW, wH / bH, 1); // chỉ thu nhỏ, không phóng to

  if (scale < 1) {
    board.style.transformOrigin = 'top left';
    board.style.transform = `scale(${scale})`;
    // Set kích thước box = nội dung tự nhiên để overflow:hidden không
    // cắt trước khi scale áp dụng.
...
function initSeating(opts = {}) {
  ...
  userGroup = opts.userGroup != null ? String(opts.userGroup) : null;
  // Reset watch-state khi khởi tạo mới để poll đầu tiên luôn fetch đầy đủ.
  _lastWatchVersion   = 0;
  _lastWatchAccStatus = '';
