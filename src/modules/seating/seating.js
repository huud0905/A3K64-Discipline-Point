...
function bindEvents() {
  ...
  if (modal === 'publish') {
    ...
    on('modal-confirm', async () => {
      ...
      if (status === 'preview') {
        ...
        if (!previewStudents) {
          showToast('Chọn ít nhất một học sinh để dùng chế độ Xem trước.');
          return;
        }
      }
  ...
  if (modal === 'manage') {
    ...
    on('modal-room-save', async () => {
      if (!canEditSeating()) { showToast('Bạn không có quyền sửa nhãn.'); return; }
...
async function handleSaveRoomConfig() {
  ...
  if (dataSource === 'gas') {
    showToast('Đang lưu cấu hình phòng...');
    const { chart, error: saveErr } = await gasSaveChart(currentId, title, seatState, roomConfig);
  ...
  render();
  showToast('Đã lưu cấu hình phòng (cục bộ — chưa kết nối Google Sheets).');
}
...
function onRoomMoveEnd() {
  ...
  roomDrag = null;
  render();
  showToast('Đã đổi vị trí — bấm "Lưu sơ đồ" để lưu lại.');
}
...
function onRoomResizeEnd() {
  ...
  roomResize = null;
  render();
  showToast('Đã đổi kích thước — bấm "Lưu sơ đồ" để lưu lại.');
}
...
async function checkAccessNow() {
  ...
  if (wasBlocked && !isBlockedNow) {
    ...
    showToast(newStatus === 'preview' ? 'Bạn vừa được thêm vào danh sách xem trước.' : 'Sơ đồ vừa được công bố.');
    render();
