...
const HISTORY_HEADERS = ["id", "week", "studentId", "studentName", "action", "before_json", "after_json", "actor", "reason", "createdAt"];
...
function addScoreEvent(payload) {
  ...
  history(week, event.studentId, info.name, "addScore", before, after, actor, title);
...
function deleteScoreEvent(eventId) {
  ...
  history(week, targetEvent.studentId, studentName, "deleteScore", before, after, "Web", targetEvent.title);
...
function history(week, studentIdValue, studentName, action, before, after, actor, reason) {
...
function readRows(name, params) {
  ...
  for (let row = 1; row < values.length; row++) {
    ...
    if (params.studentId && item.studentid !== params.studentId) continue;
